...
func NewComponent(ctx *pulumi.Context, name string, args ComponentArgs, options ...pulumi.ResourceOption) (*Component, error) {
	...
	type provider interface {
		setup(ctx *pulumi.Context, ns pulumix.Input[string], options ...pulumi.ResourceOption) (DatabaseComponent, error)
	}
	var databaseComponentProvider provider
	...
	case args.Postgres != nil:
		databaseComponentProvider = args.Postgres
	case args.RDS != nil:
		databaseComponentProvider = args.RDS
	default:
		return nil, errors.New("either Postgres or RDS config must be provided")
	}

	cmp.DatabaseComponent, err = databaseComponentProvider.setup(ctx, args.CommonArgs.Namespace, pulumi.Parent(cmp))
	if err != nil {
		return nil, err
	}
	...
	if v, ok := cmp.DatabaseComponent.(DatabaseWithOwnedServiceComponent); ok {
		// todo: make async to not block devbox install
	...
	args.DisableUpgrade.ApplyT(func(disableUpgrade bool) error {
		...
		_, err = runMigrateJob(ctx, migrationArgs{
