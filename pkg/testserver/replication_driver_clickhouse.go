...
func (h *ClickhouseDriver) ReadMessages(ctx context.Context) ([]drivers.LogWithLedger, error) {
	...
	rows, err := h.client.Query(ctx, "select ledger, id, type, date, data from logs final")
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		newLog.ID = pointer.For(int(id))

		payloadAsBytes, err := payload.MarshalJSON()
		if err != nil {
			return nil, errors.Wrap(err, "marshalling data")
		}

		newLog.Data, err = ledger.HydrateLog(newLog.Type, payloadAsBytes)
		if err != nil {
