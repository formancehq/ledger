...
var v0CreateTransaction = func(t *testing.T, store *sqlstorage.Store, tx TransactionV0) bool {
	sqlx, args := sqlbuilder.NewInsertBuilder().
		InsertInto(store.Table("transactions")).
		Cols("id", "timestamp", "hash", "reference").
		Values(tx.ID, tx.Timestamp, "xyz", tx.Reference).
		BuildWithFlavor(store.Flavor())
	_, err := store.DB().Exec(sqlx, args...)
	...
	for _, p := range tx.Postings {
		sqlx, args = sqlbuilder.NewInsertBuilder().
			InsertInto(store.Table("postings")).
...
func v0CountMetadata(t *testing.T, store *sqlstorage.Store) (int, bool) {
	sqlx, args := sqlbuilder.Select("count(*)").From(store.Table("metadata")).BuildWithFlavor(store.Flavor())
	rows, err := store.DB().Query(sqlx, args...)
	if !assert.NoError(t, err) {
		return 0, false
	}
...
func v0AddMetadata(t *testing.T, store *sqlstorage.Store, targetType, targetId, timestamp, key string, value json.RawMessage) bool {
	...
	sqlx, args := sqlbuilder.NewInsertBuilder().
		InsertInto(store.Table("metadata")).
...
func TestMigrates(t *testing.T) {
	...
	s, err := driver.NewStore(uuid.New())
	if !assert.NoError(t, err) {
		return
	}
	store := s.(*sqlstorage.Store)
	flavor := strings.ToLower(store.Flavor().String())
