...
* [RevertTransaction](#reverttransaction) - Revert a transaction
...
Retrieves a transaction by its ID from the specified ledger
...
Reverts a transaction by creating a reverse transaction that undoes its effects
...
func main() {
    ...
    res, err := s.Transactions.RevertTransaction(ctx, operations.RevertTransactionRequest{
        LedgerName: "<value>",
        TransactionID: 102914,
    })
    if err != nil {
        log.Fatal(err)
    }
    if res.RevertedTransactionResponse != nil {
        // handle response
    }
}
```
