...
func TestPushPublishesTheOneReviewedCandidate(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, "1", strings.TrimSpace(string(countBytes)))
}
...
func TestPushRefusesWhenReviewedCandidateHeadMoves(t *testing.T) {
	...
	require.Contains(t, output, "candidate review left the worktree dirty")
...
`), 0o644))
	reviewLoop := strings.Join([]string{
		"#!/usr/bin/env bash",
		"set -euo pipefail",
		"review_cmd=\"\"",
		"validation_cmd=\"\"",
		...
		"esac",
		"review_words=()",
		"eval \"review_words=($review_cmd)\"",
		"[[ \"${review_words[1]}\" == *trusted-tools/scripts/agent-validation-env ]] || exit 94",
		"review_script=${review_words[$((${#review_words[@]} - 1))]}",
		...
		"            ;;",
		"    esac",
		"    if [[ \"${TEST_MOVE_REMOTE:-false}\" == \"true\" ]]; then",
		"        git --git-dir=\"$TEST_REMOTE\" update-ref refs/heads/feature \"$TEST_BASE_SHA\"",
		"    elif [[ \"${TEST_MOVE_LOCAL_HEAD:-false}\" == \"true\" ]]; then",
		"        printf 'unreviewed change\\n' > unreviewed.txt",
		"    elif [[ \"${TEST_TARGET_MUTATION:-}\" == \"after-exact-review\" ]]; then",
		"        git --git-dir=\"$TEST_REMOTE\" update-ref refs/heads/release/v3.0 \"$TEST_ADVANCED_BASE_SHA\"",
		"    fi",
		"fi",
		"exit 0",
...
`)
	...
	if options.tamperTargetToolchain {
		runGit(t, seed, "add", "scripts/review-loop", "scripts/ai-review-codex", "scripts/ai-pr-known-findings", "scripts/agent-check-pr")
	}
