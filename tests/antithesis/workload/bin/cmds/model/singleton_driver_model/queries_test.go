...
func TestAccountMatches_RejectsColouredVolumes(t *testing.T) {
	...
	acct.Volumes = append(acct.Volumes, &commonpb.AccountVolume{
		Asset: "USD", Color: "red",
		Volumes: &commonpb.VolumesWithBalance{Input: commonpb.MustBigUintFromDecimal("1"), Output: commonpb.MustBigUintFromDecimal("0"), Balance: commonpb.MustSignedBigIntFromDecimal("1")},
	})
	require.False(t, accountMatches(ls, "acc:1", acct))
}
