...
func describeAccountContentDiff(ls oracle.LedgerState, addr string, serverAcct *commonpb.Account) string {
	...
	for _, av := range serverAcct.GetVolumes() {
		...
		if !ok {
			return fmt.Sprintf("%s volumes[%s] model=<absent> server=in:%s,out:%s", addr, av.GetAsset(), av.GetVolumes().GetInput().DecimalString(), av.GetVolumes().GetOutput().DecimalString())
		}

		if vp.Input.Dec() != av.GetVolumes().GetInput().DecimalString() || vp.Output.Dec() != av.GetVolumes().GetOutput().DecimalString() {
			return fmt.Sprintf("%s volumes[%s] model=in:%s,out:%s server=in:%s,out:%s", addr, av.GetAsset(), vp.Input.Dec(), vp.Output.Dec(), av.GetVolumes().GetInput().DecimalString(), av.GetVolumes().GetOutput().DecimalString())
		}
	}
