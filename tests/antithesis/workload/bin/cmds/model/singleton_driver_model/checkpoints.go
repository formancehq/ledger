...
// frozen read.
func runCheckpointCycle(ctx context.Context, c *Checker, client servicepb.BucketServiceClient) {
	...
	for {
		...
		if len(records) < maxTrackedCheckpoints && random.RandomChoice([]uint8{0, 1}) == 0 {
			if rec := createAndFingerprint(ctx, c, client); rec != nil {
		...
		case probeOK:
			...
			if rec.probes >= 3 {
				...
				_ = actions.DeleteQueryCheckpoint(dctx, client, rec.id)
				cancel()
...
// the round.
func createAndFingerprint(ctx context.Context, c *Checker, client servicepb.BucketServiceClient) *checkpointRecord {
	cctx, cancel := context.WithTimeout(ctx, 15*time.Second)
	checkpointID, _, err := actions.CreateQueryCheckpoint(cctx, client)
	...
	rec := &checkpointRecord{id: checkpointID, ledger: random.RandomChoice(c.ledgerNames)}
