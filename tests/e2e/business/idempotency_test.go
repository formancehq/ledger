...
var _ = Describe("Idempotency Keys", Ordered, func() {
	...
	Context("When using idempotency keys for transactions", Ordered, func() {
		...
		It("Should allow same content with different idempotency keys", func() {
			...
			Expect(err).To(Succeed())
			Expect(account.FindVolume("USD", "").GetInput().DecimalString()).To(Equal("200"))
		})
	})
	...
	Context("When using idempotency keys in bulk operations", Ordered, func() {
		...
		It("Should handle idempotency across separate keyed batches", func() {
			...
			Expect(err).To(Succeed())
			Expect(account1.FindVolume("USD", "").GetInput().DecimalString()).To(Equal("100"))
			...
			Expect(err).To(Succeed())
			Expect(account2.FindVolume("USD", "").GetInput().DecimalString()).To(Equal("200"))
		})
	})
})
