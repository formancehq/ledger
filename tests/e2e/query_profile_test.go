	...
	Context("ListAccounts profiling", Ordered, func() {
		...
		It("Should return profile in trailing metadata when requested", func() {
			Eventually(func(g Gomega) {
				profileCtx := metadata.AppendToOutgoingContext(ctx, "x-query-profile", "true")

				stream, err := client.ListAccounts(profileCtx, &servicepb.ListAccountsRequest{
					Ledger: ledgerName,
				})
	...
	Context("ListTransactions profiling", Ordered, func() {
		...
		It("Should return profile in trailing metadata when requested", func() {
			Eventually(func(g Gomega) {
				...
				stream, err := client.ListTransactions(profileCtx, &servicepb.ListTransactionsRequest{
					Ledger: ledgerName,
				})
				g.Expect(err).To(Succeed())
	...
	Context("ExecutePreparedQuery profiling", Ordered, func() {
		...
		It("Should return profile in trailing metadata when requested", func() {
			Eventually(func(g Gomega) {
				...
				resp, err := client.ExecutePreparedQuery(profileCtx, &servicepb.ExecutePreparedQueryRequest{
					Ledger:    ledgerName,
		...
		BeforeAll(func() {
			...
			_, err = client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					...
					createTransactionAction(ledgerName, []*commonpb.Posting{
						newPosting("world", "merchants:shop1", big.NewInt(50), "USD"),
					}, nil, nil),
				},
			})
			Expect(err).To(Succeed())
