...
var _ = Describe("LedgerctlTypedMetadata", Ordered, func() {
	...
	Context("ledgers get-schema", Ordered, func() {
		...
		It("Should show empty schema for a ledger without schema", func() {
			...
			_, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{createLedgerAction(emptyLedger, nil)},
			})
			Expect(err).To(Succeed())
	...
	Context("Full lifecycle via CLI", Ordered, func() {
		...
		It("Should enforce typed metadata on account writes", func() {
			_, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					saveAccountMetadataAction(ledgerName, "user1", map[string]string{
						"age":    "25",
						"active": "true",
						"score":  "100",
					}),
			...
			// active → bool_value
			activeVal := findMetadataValue(account.Metadata, "active")
			...
			// score → uint_value
			scoreVal := findMetadataValue(account.Metadata, "score")
	...
	Context("CLI with background conversion monitoring", Ordered, func() {
		...
		BeforeAll(func() {
			...
			_, err = client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					saveAccountMetadataAction(ledgerName, "user1", map[string]string{"score": "42"}),
					saveAccountMetadataAction(ledgerName, "user2", map[string]string{"score": "99"}),
				},
			})
			Expect(err).To(Succeed())
		})
