...
var _ = Describe("PreparedQueries", Ordered, func() {
	...
	Context("Execute LIST mode — account metadata string filter", Ordered, func() {
		...
		It("Should paginate with page_size=1", func() {
			...
			Eventually(func(g Gomega) {
				var err error
				firstPage, err = client.ExecutePreparedQuery(ctx, &servicepb.ExecutePreparedQueryRequest{
					Ledger:    ledgerName,
					QueryName: "find-admins",
					Mode:      commonpb.QueryMode_QUERY_MODE_LIST,
					PageSize:  1,
			...
			secondPage, err := client.ExecutePreparedQuery(ctx, &servicepb.ExecutePreparedQueryRequest{
				Ledger:    ledgerName,
				QueryName: "find-admins",
				Mode:      commonpb.QueryMode_QUERY_MODE_LIST,
				PageSize:  1,
	...
	Context("Execute LIST mode — AND/OR/NOT filters", Ordered, func() {
		...
		BeforeAll(func() {
			...
			_, err = client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					saveAccountMetadataAction(ledgerName, "alice", map[string]string{"role": "admin", "tier": "premium"}),
					saveAccountMetadataAction(ledgerName, "bob", map[string]string{"role": "user", "tier": "premium"}),
					saveAccountMetadataAction(ledgerName, "charlie", map[string]string{"role": "admin", "tier": "basic"}),
					saveAccountMetadataAction(ledgerName, "diana", map[string]string{"role": "user", "tier": "basic"}),
				},
			})
			Expect(err).To(Succeed())
		})
		...
		BeforeAll(func() {
			...
			_, err = client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					...
					createForceTransactionAction(ledgerName, []*commonpb.Posting{
						newPosting("world", "alice", big.NewInt(50), "EUR"),
					}, nil),
					createForceTransactionAction(ledgerName, []*commonpb.Posting{
						newPosting("world", "bob", big.NewInt(200), "USD"),
					}, nil),
