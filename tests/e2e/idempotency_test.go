	...
	Context("When using idempotency keys for ledger creation", func() {
		It("Should create a ledger with idempotency key", func() {
			resp, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(createLedgerAction("idempotent-ledger", nil), "create-ledger-key-1"),
				},
			})
		...
		It("Should return same result for duplicate request with same idempotency key", func() {
			...
			resp1, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(createLedgerAction("duplicate-ledger", nil), idempotencyKey),
				},
			})
			Expect(err).To(Succeed())
		...
		It("Should fail when reusing idempotency key with different content", func() {
			...
			resp1, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(createLedgerAction("ledger-a", nil), idempotencyKey),
				},
			})
			Expect(err).To(Succeed())
			Expect(resp1).NotTo(BeNil())
			...
			_, err = client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(createLedgerAction("ledger-b", nil), idempotencyKey),
				},
			})
		...
		It("Should return same result for duplicate transaction with same idempotency key", func() {
			...
			resp1, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(
						createTransactionAction(ledgerName, []*commonpb.Posting{
							newPosting("world", "account-dup", big.NewInt(100), "USD"),
						}, nil, nil),
						idempotencyKey,
					),
				},
		...
		It("Should fail when reusing idempotency key with different transaction content", func() {
			...
			resp1, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(
						createTransactionAction(ledgerName, []*commonpb.Posting{
							newPosting("world", "account-conflict", big.NewInt(100), "USD"),
						}, nil, nil),
						idempotencyKey,
					),
				},
			...
			_, err = client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(
						createTransactionAction(ledgerName, []*commonpb.Posting{
							newPosting("world", "account-conflict", big.NewInt(200), "USD"),
						}, nil, nil),
						idempotencyKey,
					),
				},
		...
		It("Should allow same content with different idempotency keys", func() {
			...
			resp1, err := client.Apply(ctx, &servicepb.ApplyRequest{
				Requests: []*servicepb.Request{
					withIdempotencyKey(
						createTransactionAction(ledgerName, []*commonpb.Posting{
							newPosting("world", "account-multi", big.NewInt(100), "USD"),
						}, nil, nil),
						"key-a",
					),
				},
