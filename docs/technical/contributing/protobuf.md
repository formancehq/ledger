...
| `internal/application/usagebuilder/protowire_order.go` | read | **hardcoded literals** across `Order` (ledger/system oneof 1–2, technical 3), `LedgerScopedOrder` (ledger 1 and payload oneof), `LedgerApplyOrder` (1–11), `CreateTransactionOrder` (1–8), `Script` (1–3), `NumscriptReference` (1–3), and `Timestamp.data` (1) | Breaks selective native-transaction usage decoding or oneof recognition. Update the scanner, oneof membership tables, and canonical-decoder comparison tests whenever any listed message is renumbered. |
