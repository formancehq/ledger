...
| `GET /v3/_/events-sinks` | ✅ | ❌ | List configured event sinks with per-sink status (`{sinks, sinkStatuses}`, both arrays sorted by name). **Not at parity with gRPC `GetEventsSinks` on sink credentials:** the HTTP response DTOs (`internal/adapter/http/dto_sinks.go`) declare no field for Kafka `saslPassword`, HTTP `secret`, ClickHouse `dsn`, Databricks `token` or OAuth `clientSecret`, so those five values cannot be serialized on this route (EN-1791). This is not blanket credential safety: `nats.url` and `http.endpoint` are returned verbatim including any userinfo they embed, and URL userinfo is the only auth channel a NATS sink has. gRPC `GetEventsSinks` returns all five secrets in plaintext to any caller holding `ScopeOpsRead`. Every other field is at parity |
| `GET /v3/_/signing-keys` | ✅ | ❌ | List registered Ed25519 signing keys, sorted by `keyId` (the upstream source is a Go map, so the gRPC order is not stable across calls). **Not at parity with gRPC on `publicKey` encoding:** the HTTP response DTO (`internal/adapter/http/handlers_list_signing_keys.go`) emits lower-hex, matching every other byte field on this API and `ledgerctl signing list-keys`; gRPC returns the raw bytes, which protojson renders as base64 (EN-1791). The hex contract is pinned in `openapi.yml` by `SigningKey.publicKey`'s `pattern: '^[0-9a-f]*$'` |
