...
| `GET /v3/_/events-sinks` | ✅ | ❌ | List configured event sinks with per-sink status (`{sinks, sinkStatuses}`, both arrays sorted by name). **Not at parity with gRPC `GetEventsSinks` on sink credentials:** the HTTP response DTOs (`internal/adapter/http/dto_sinks.go`) declare no field for Kafka `saslPassword`, HTTP `secret`, ClickHouse `dsn`, Databricks `token` or OAuth `clientSecret`, so those five values cannot be serialized on this route (EN-1791). gRPC `GetEventsSinks` returns all five in plaintext to any caller holding `ScopeOpsRead`. Every other field is at parity |
