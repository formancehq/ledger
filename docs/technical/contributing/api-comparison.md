...
| Inspect index | ✅ | ❌ | Explore values of a metadata index (distinct values, facets, summary). HTTP: `GET /v3/{ledger}/indexes/{canonicalId}/inspect`; gRPC: `InspectIndex`. Values are typed by the served binding (`served_type` on the gRPC response); a binding outside the schema's serving window (replaced lineage, or more than one revision behind) refuses as retryable `INDEX_BUILDING` |
