...
business RPC, equal to `pkg/grpcprotocol.Version` (currently `"11"`). Revision 11
requires the target's 16-byte `instance_id` on administrative `AddLearner`
requests; revision 10 clients that omit it are incompatible. Missing,
...
| Revert target created in the same batch | `INVALID_ARGUMENT` | `REVERT_TARGET_CREATED_IN_BATCH` | `transactionId` |
