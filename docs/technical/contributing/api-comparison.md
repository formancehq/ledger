...
**Two axes, deliberately separate.** The semantic `ErrorKind` answers the client; the gRPC status code is a transport-behaviour signal that drives retry and hop behaviour. They may intentionally disagree, and the receiver preserves both: the kind through the `Descriptor`, the exact original status through the decoder's carrier (`GRPCStatus()`). `READ_INDEX_NOT_CAUGHT_UP` is the live example — semantically `KindUnavailable` (`503` + `Retry-After` on REST) while travelling as `codes.FailedPrecondition`, because `actions.GRPCRetryPolicy` retries `codes.Unavailable` fifty times at 0.2s and the semantic code would turn a read-index lag into a ten-second client-side hang. Do not reconstruct a status from a `Kind`; it is lossy in both directions.

**Mismatch policy.** A reason ledger knows is a reason whose legitimate wire codes it knows: `CodeForKind(KindForReason(reason))` plus any declared per-reason exception (today only the `READ_INDEX_NOT_CAUGHT_UP` pair above). Validation is therefore reason-specific rather than kind-specific. A known reason arriving under a code outside that set is a protocol fault, not a business outcome: it is recorded server-side and passes through the normal internal-error sanitizer (`500 INTERNAL_ERROR` + correlation ID on REST, `codes.Unknown` + correlation ID on gRPC), and the received reason, message and metadata are dropped rather than answered as trusted business information. An unknown reason cannot be validated and is preserved verbatim — reason, message, metadata and exact status.
