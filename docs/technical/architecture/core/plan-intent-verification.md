...
Under concurrent write pressure, a `DeleteMetadata` on a key admission had sampled as absent could violate `AttributeCache.Del`'s contract (the cache diverges from the pre-refactor Gen0-strict expectation):
...
T2  Rotation fires (raftIndex crosses cache-rotation-threshold)
      ...
      → new Gen0 = ∅
      → net state: Gen1[id] = v, Gen0[id] = ∅

T3  Delete(X.k) applies at raftIndex N > M
      processDeleteMetadata → AttributeCache.Del sees Gen0[id] = ∅
      → pre-fix: PANIC (Gen0-strict contract)
```
