...
1. **single-flights** the load per key (`loading map[attributes.U128]*inflightLoad`), so concurrent proposals resolving the same key perform one Pebble read and observe the same value;
2. **memoizes** the loaded value (`loaded map[attributes.U128]*loadedEntry[T]`), validity keyed by a `preload.CacheStamp` — the generation boundary, the replicated cache epoch and the local reset count (`ConfigSnapshot.ResetSeq`, bumped by every clear of the cache, snapshot install and restore included) — so a later proposal in the same generation reuses it and no value read before a reset survives one;
3. **shards its state** (`loading`, `loaded`, the `fenced map[attributes.U128]int` per-key fence counts and the `fencedAll` count) 256 ways, keyed by `U128.Lo()`, to avoid mutex contention — the sharding is over the loader's maps, not over the cache.

`Release()` deletes the memoized entry — it does **not** decrement a refcount, and it **evicts nothing** from the attribute cache, whose entry survives for as long as the rotation policy allows. Invalidation happens at two points. The FSM brackets every proposal's commit with a fence on its covered keys (`state.PreloadInvalidator`, wired to `plan.Builder.FencePreloaded` / `UnfencePreloaded` in bootstrap): `Fence` drops the memo immediately before `pb.batch.Commit()`, and no load that starts, runs or completes while the fence is up is memoized — its Pebble read may sit on either side of the write — until the matching `Unfence`, which runs once the commit has returned (or failed). So no preload is served a memoized value from before a committed write to a key the proposal covered. The one write outside a proposal's covered keys is `DeleteLedger`'s ledger-wide range delete (`batch.DeleteLedgerData`), whose keys no plan can enumerate; the commit of a batch that deletes a ledger is bracketed by `FenceAllPreloaded` / `UnfenceAllPreloaded`, which fence every loader. The proposer's `CleanupToken` releases again when its handler returns, which covers proposals that never applied. Every attribute type, the index registry included, takes the same path: a cache hit (same tag) is coverage-only, a bloom-absent key is coverage-only, and everything else loads through the loader.
