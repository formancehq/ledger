...
The idempotency TTL is applied without ever re-reading the policy in the expiry check: each apply freezes the outcome's absolute `expires_at` once, as `created_at + IdempotencyTtlMicros` from the policy committed *before* that proposal, and stores it on the outcome (and chain-binds it in the audit header). Every later decision reads only committed state — `IsExpired` and the leader eviction scan compare the stored `expires_at` against the HLC / the eviction cutoff — so a node whose desired TTL differs (mid-rollout, or misconfigured) cannot diverge a committed outcome's lifetime. Reading the live policy TTL inside the expiry check instead would let two nodes disagree on whether the same committed outcome is still live (EN-1797).

The preload re-injection gate is the one place that must not use the HLC. An `IdempotencyEviction` is a technical-only proposal, so it never advances `LastAppliedTimestamp`; on an idle cluster the HLC lags the eviction's wall-clock cutoff, and a proposal whose plan predates the eviction would re-inject the already-removed outcome into the cache (map ahead of Pebble). The gate therefore consults a separate committed high-water mark, `FSMState.LastIdempotencyEvictionCutoff` (advanced by `applyIdempotencyEviction`, persisted at `ZoneGlobal`/`SubGlobLastIdempotencyEvictionCutoff`): the gate skips re-injecting any outcome whose `expires_at` is at or below it. This cutoff is a scan horizon, not a proof that every such outcome is already gone — a tick that finds more than `maxEvictionBatchSize` expired entries advances it to the full wall-clock now while removing only the first batch, leaving the rest live below the cutoff until a later tick. Correctness rests on the map-superset invariant, not on the cutoff being exact: a still-live outcome is always present in the in-memory map (eviction removes its map and Pebble entries in one atomic batch; restart and snapshot restore rebuild the map from Pebble), so skipping its re-injection is a no-op. Because the cutoff is committed FSM state, replay and snapshot restore reach the same decision on every node.
...
safety properties without changing the existing error behavior. Pebble read
errors remain I/O failures and do not assert missing data. The `nonempty sentinel
