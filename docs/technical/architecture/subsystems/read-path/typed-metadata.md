...
index as `INDEX_BUILDING`, and `Store.FlushServingTransitions` clears the mark
...
`InspectIndex` pins a read-store snapshot, rejects `CurrentVersion == 0` and a
promotion whose flush is still in flight, and scans only the locally served
version. Distinct values, facets, and summary
