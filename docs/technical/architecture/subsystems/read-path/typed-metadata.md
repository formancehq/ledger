...
bindings from a replaced declaration lineage or more than one revision behind
as `INDEX_BUILDING` (`ClassifyBindingWindow` — the same decision the query
compile gate makes), and scans only the locally served version. Distinct values, facets,
