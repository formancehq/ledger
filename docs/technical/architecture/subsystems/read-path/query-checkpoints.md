...
8. **One open serves every concurrent reader of a checkpoint.** Pebble takes a
   ...
   - **Readers waiting on another reader's open honor their context**, and the
     ...
     arriving afterwards opens again.
...
- **A damaged directory fails permanently instead of retrying forever.** Both markers present means each half was complete when it was published, so a read-only open that then fails is damage, not lag: the error surfaces as-is and reaches the client as a sanitized permanent `Unknown` carrying a correlation ID (the node log holds the cause), never the retryable `Unavailable` that `actions.GRPCRetryPolicy` would retry 50 times. Recovery is the same delete-and-recreate as the cases below. Readers already waiting on that open receive the same error; the shared entry is withdrawn as the error publishes, so the next reader to arrive opens again (step 8). A transient cause such as a descriptor or disk-space limit is not distinguished from damage.
- **A panicking open is a failed read, not a wedged checkpoint.** Pebble v2.1.4 releases its directory lock while unwinding an open-time panic, and a panic out of the read-index open unwinds the main store the same call already opened. The recovered panic is logged with its stack and surfaces as an error to the readers sharing that open; the entry is withdrawn as it publishes, so a later reader opens again. No node restart is needed.
