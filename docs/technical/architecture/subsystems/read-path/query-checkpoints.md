...
7. Both stores are opened read-only when a query specifies `checkpoint_id`, and
   ...
   index rather than trusting `.ready` alone. Opening the pair also acquires a
   node-local filesystem lease before checking readiness. A committed deletion
...
8. **One open serves every concurrent reader of a checkpoint.** Pebble takes a
   ...
   - **Readers waiting on another reader's open honor their context**, and the
     ...
     inside `pebble.Open` is unwound by Pebble itself: the pinned v2.1.4
     implementation defers releasing its directory lock before propagating the
     panic. Ledger logs the recovered panic and returns an error to affected
     readers; the shared entry is withdrawn as that error publishes, so a reader
     arriving afterwards opens again rather than inheriting the failure.
...
- **A damaged directory fails permanently instead of retrying forever.** Both markers present means each half was complete when it was published, so a read-only open that then fails is damage, not lag: the error surfaces as-is and reaches the client as a sanitized permanent `Unknown` carrying a correlation ID (the node log holds the cause), never the retryable `Unavailable` that `actions.GRPCRetryPolicy` would retry 50 times. Recovery is the same delete-and-recreate as the cases below. Because the readiness markers are checked before the open, the expected failure past that gate is damage rather than lag, and a second open would hit it identically: readers already waiting on that open are served the same error, and the entry is withdrawn as the error publishes, so the next reader to arrive opens again (step 8). A transient cause — a descriptor or disk-space limit — is not distinguished from damage, so it costs the readers already waiting an error each before the next reader opens again.
- **A panicking open remains a visible failed read.** Pebble v2.1.4 releases its directory lock while unwinding an open-time panic. Ledger closes any main-store handle already returned before a read-index open fails, logs the recovered panic with its stack, and returns an error to the readers sharing that open. After those readers release the entry, a later reader can attempt a fresh open. A panic alone does not establish a retained lock or require a node restart; investigate the concrete error and cleanup path.
