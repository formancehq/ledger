...
- **A directory left locked by a panicking open needs a restart.** Pebble takes the directory lock inside `pebble.Open`, before it returns a handle, so a panic in there leaves the path in Pebble's process-global lock table with nothing for the caller to close (step 8). The shared entry is evicted, so later reads of that checkpoint open again and fail with `lock held by current process` — surfaced, like damage, as a permanent `Unknown` with a correlation ID. The node log distinguishes it: the recovered panic is logged with its stack at the open, and each later failure logs the lock message. It is node-local and resolved by restarting that node; deleting and recreating the checkpoint does not clear the lock, because the entry is keyed by path.
