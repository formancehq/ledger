...
4. The Applier materializes the main store at `{dataDir}/query-checkpoints/{id}/main/`, **per-replica** and **atomic** on the same pattern as the read index below: build into `main.tmp/`, fsync, rename, then write `.ready` last. `pebble.DB.Checkpoint` writes the MANIFEST that makes a directory openable *before* it copies the WAL files, so a directory caught mid-copy opens cleanly while missing every write still resident in the memtable; the marker, written only after the rename, is what vouches for completeness. A redundant materialization on an already-marked directory is a no-op, which the Applier relies on when it re-crosses the checkpoint log during spool or WAL replay.
