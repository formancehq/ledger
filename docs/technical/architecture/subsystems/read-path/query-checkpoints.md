...
- **Transient live materialization failures are retried before later mutations.** The projection batch and native cursor commit before filesystem materialization. If materialization then fails, the builder retains the checkpoint id and audit horizon in memory and retries that side effect at the start of the next `processLogs` call. It does not replay the committed batch, which could duplicate lifecycle mutations. Later logs, backfills, and event GC remain behind this retry boundary so the frozen readstore still represents the original checkpoint horizon.
- **Unrecoverable checkpoints degrade to `NotFound`, not wrong data.** There is no historical reconstruction: inline materialization is already exactly point-in-time. If a node crashes between the projection commit and the `.ready` marker, the in-memory retry state is lost and that node will never have a `.ready` marker for the checkpoint. Since the checkpoint is still registered, reads there return the retryable `Unavailable` and never self-heal — the operator/client recreates the checkpoint (aligned with the existing `AcquireCheckpoint` client workaround, which deletes-and-recreates on timeout). Deleting the checkpoint then makes reads return `NotFound`.
- **Deletion racing with a read-index open is classified from the original open error.** Physical cleanup removes `readindex/` before `main/`, then removes the checkpoint parent directory. During recursive read-index removal, database files can disappear while `.ready` still exists; after read-index removal completes, `main/` still exists until the next cleanup step. After the marker check and main-store open succeed, a read-index open failure becomes `NotFound` only when the error indicates missing files and a fresh, consistent main-store snapshot proves deletion: the registry row is absent and the next checkpoint ID exceeds the requested ID. Missing files for a live checkpoint, corruption, and unrelated permission or I/O failures remain errors. A failed lifecycle lookup returns retryable `Unavailable` and reports both the read-index opening and lifecycle-reading stages. The main store is closed on every read-index open failure. The read-only VFS wrapper preserves the original `OpenDir` cause that Pebble otherwise discards; a later filesystem stat cannot replace that evidence because deletion may have occurred after an unrelated failure.
- **A successful frozen read does not prove the checkpoint is still registered.** Checkpoint reads skip the live-read barrier and do not consult the registry when both stores open successfully. A replica that has not applied deletion, or a read that opens the stores before filesystem cleanup, can therefore return the original frozen data after the deletion response. Once the local files are gone, the missing-marker path establishes the linearizable lifecycle evidence before returning `NotFound`. A live-read fence alone does not wait for filesystem cleanup.
