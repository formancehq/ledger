...
- **Missing or corrupt state**: the builder fails the replay invariant; it never silently treats an unproved ledger as empty. This terminal state wakes projection/checkpoint waiters with `ErrReadProjectionFailed` and makes the node's gRPC health report `NOT_SERVING`, so the process cannot remain apparently healthy with a permanently stale read projection.
