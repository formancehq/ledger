...
**Both paths flush the read store after the commit, and the promotion is not served until that flush completes.** The read store has no WAL, so the committed switch lives only in the memtable until Pebble flushes it; a hard kill in that window reopens the store at the pre-switch state. Unlike a fold-inline transition (born-empty initial version, drop tombstone), a switch is not re-ordered by the fold cursor after a rewind — the fold catches up while the rewrite or backfill is redone, so the node would serve the superseded binding at states where it had already served the promoted one. The order on every promotion path is: `writePromotion` writes the promoted state into the batch; the old version's GC is queued into the same batch (rewrite paths only); `commitPromotion` marks the index in flight in the read store and then commits the batch — the mark immediately precedes the commit, so readers keep serving the standing version while the batch is assembled and no reader can observe a committed switch that is not refused; `finishPromotion` publishes the state to the builder's cache and calls `readstore.Store.FlushPromotions`, which flushes Pebble and releases the mark. Readers (`PinnedVersionResolver`, `InspectIndex`) refuse a marked index as `INDEX_BUILDING`; the status endpoints (`GetIndexStatus`, `GetIndexEntryStatus`) report `current_version` without that gate, so a poller can see the promoted version and, after a kill in the window, the superseded one again — no rows are ever served under it. The refusal window is mark-to-flush, the mark being placed immediately before the commit (`commitPromotion`); the `InFlight` consult is against the live tracker, so during that window a read whose snapshot predates the switch is refused too, although `v_old` was still servable for it — the price of never serving a committed switch that is not yet durable. The mark's lifecycle follows the commit, not the flush: a commit that fails withdraws its mark (`commitPromotion`), a batch released before its commit never carried one (`discardBatch`), while a flush that fails after a successful commit leaves the promotion committed and cached, keeps the mark, and is logged — the index stays refused and `retryPromotionFlush`, run at the top of every steady-state loop wake, whether a commit signal or the fallback ticker woke it (the boot-time `processBackgroundTasks` call precedes the first retry, so a flush that fails there is retried on the first wake after catch-up), re-attempts the flush until one succeeds (a live Pebble store retries a failed memtable flush internally and returns only on success, so no such error is expected; the path exists so a returned error can never strand a committed promotion behind a permanent refusal). Marks are counted per index, so withdrawing a later promotion that never committed cannot lift the gate an earlier committed one still needs. A tick with no mark pending triggers no flush, so ordinary ticks pay nothing; a promotion itself flushes synchronously on the builder goroutine, so the fold batch after it — and the Raft certificate it publishes — waits for that flush.
...
| Crash *after* the switch committed, *before* its flush completed | Either nothing of the switch — the memtable is lost, so `IndexVersionState` reopens at `{Current: old, Pending: new}` with the pre-switch cursor — or, if Pebble rotated the memtable on its own before the kill, the switch itself. No query observed the promotion either way — the in-flight mark had readers refusing it. | If the switch was lost, boot re-registers the rewrite (or, with `CurrentVersion == 0`, the backfill) from the surviving `PendingVersion`; the gate is re-evaluated and the switch fires again. If Pebble had rotated the memtable, the state reopens as `{Current: new, Pending: 0}`, nothing is re-registered and the promotion is complete. |
| Crash *after* the switch's flush completed | The switch itself, its same-batch v_old GC, and every fold write before it. A state a query could have observed is never rewound past. | Nothing for the switch; the fold resumes from its flushed cursor and live writes target the promoted version. |
