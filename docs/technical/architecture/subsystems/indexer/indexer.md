...
**Both paths flush the read store after the commit, and the promotion is not served until that flush completes.** The read store has no WAL, so the committed switch lives only in the memtable until Pebble flushes it; a hard kill in that window reopens the store at the pre-switch state. Unlike a fold-inline transition (born-empty initial version, drop tombstone), a switch is not re-ordered by the fold cursor after a rewind — the fold catches up while the rewrite or backfill is redone, so the node would serve the superseded binding at states where it had already served the promoted one. `stagePromotion` marks the index in flight in the read store before the write; readers (`PinnedVersionResolver`, `InspectIndex`) refuse a marked index as `INDEX_BUILDING`; once the batch has committed, `finishPromotion` publishes the state to the builder's cache and calls `readstore.Store.FlushServingTransitions`, which flushes Pebble and clears the mark. The mark's lifecycle follows the commit, not the flush: a batch that is cancelled or fails to commit withdraws its mark (`abandonPromotion`), while a flush that fails after a successful commit leaves the promotion committed and cached, keeps the mark, and is logged — the index stays refused and `retryServingFlush`, run at the top of every builder tick, re-attempts the flush until one succeeds. A tick with no mark pending triggers no flush, so ordinary operation pays nothing.
...
| Crash *after* the switch committed | The switch itself — it is flushed before it is served, so a state a query could have observed is never rewound past | Nothing for the switch; the fold resumes from its flushed cursor and live writes target the promoted version. |
