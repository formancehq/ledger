...
>
> The window is bounded to ONE schema revision (`IndexVersionState.CurrentRevision == MetadataFieldSchema.revision - 1`). A binding further behind is not a live window but a rewound read store re-walking the retype chain (the read index is WAL-less; a hard kill rewinds it to the last flush, and the re-fold rebuilds from the `CreatedIndexLog` stamp forward). Serving it would time-travel query semantics at a pin far past the retypes, so the compile gate refuses it as `INDEX_BUILDING`, and the routing layer forwards the refusal to the leader so a converged replica answers (see [indexes.md — Per-Replica Version State](indexes.md#per-replica-version-state)).
