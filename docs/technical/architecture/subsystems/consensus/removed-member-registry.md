...
1. Validate the required member identity, then call `rawNode.ApplyConfChange(cc)` — irreversible live tracker mutation; returns the new `ConfState`.
2. `wal.UpdateSnapshotConfState(cs)` — WAL write, ordered before Pebble per EN-1413.
3. `membership.UnregisterAndBlacklist(nodeID, instanceID, removedAt)` performs a Pebble batch atomic between:
...
- Administrative and bootstrap admission never pre-wire transport or service addresses. The existing voters can commit a learner addition without that learner. The committed-change observer updates the cache and attempts endpoint wiring before resolving the successful proposal; connection/probe failures remain logged and nonfatal, so success confirms committed membership rather than reachability. Rejected admission preserves the prior connections. A submitted proposal may still commit after RPC cancellation, in which case its committed routing takes effect.
- `Membership.Set` removes and re-adds transport connections when an `UpdateNode` changes the Raft address. A service-address-only change uses the service pool's address-aware replacement and retains the existing Raft connection, avoiding an unnecessary TLS probe that could strand a healthy Raft peer.
