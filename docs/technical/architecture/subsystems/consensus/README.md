...
| [removed-member-registry.md](removed-member-registry.md) | Universal member-identity invariant and replicated `(nodeID, instanceID)` set that prevents a removed member from silently rejoining and being auto-promoted; force-removal write ordering and durable restart states. |
