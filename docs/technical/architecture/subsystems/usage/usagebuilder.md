...
One pass does read a peer store — `compareReverseMapOrphans` scans the readstore's reverse map (`0x03`), invariant #8's single deliberate exception — but it does not weaken the argument above. It exists because a missed point-delete in that one limb is a *permanent* divergence that no rebuild path repairs, which is precisely what is not true of the usagestore: every counter is reconverged by the online fold and by `usagestore.Reset()` on rollback.
