...
3. `partitionVolumes` classifies `newKept` while the account persistence policy is available. For normal/default accounts, only an undefined `Old` is new: a defined `{0, 0}` may be a real persisted row. For ephemeral accounts, an undefined or all-zero `Old` is new because an ephemeral row cannot persist at zero. `makeNewKeptKeySet(partResult.newKept)` turns that classification into the annotation set.
...
The classification "new vs existing" is decided by the preloaded prior value at merge time together with the account's persistence policy. `Update.Old.IsDefined()` alone distinguishes a fresh normal/default key from a persisted row, including a legitimate persisted `{0, 0}` row. The zero-placeholder check remains valid only for ephemeral/transient lifecycle classification, where zero rows are deliberately absent from primary storage. This is safe **because volume and ledger-type preloads are structurally required by the FSM**: balance checks, Uint256 arithmetic, persistence partitioning, and numscript resolution all read those declared values. That contract is documented as invariants #6 and #9 in AGENTS.md.
...
The checker (invariant #8) verifies projections persisted **in the primary Pebble store** — the store it operates on: `Volume`, `Metadata`, `Transaction`, `Reference`, `Boundary`, idempotency outcomes, the index registry, and the exclusion projection above. The usagestore is a **distinct, peer secondary Pebble instance** (`<data-dir>/usage/`) holding a *derived cache* of counters, rebuilt from the permanent audit chain by the boot/tick fold. The fold resumes from the last explicitly flushed cursor; an empty or wiped store replays from audit sequence 0. If a primary restore leaves the usage cursor ahead of the new audit head, the builder automatically performs a durably flushed `usagestore.Reset()` before replaying from zero. Its authoritativeness is explicitly bounded by "eventually consistent with the FSM".
