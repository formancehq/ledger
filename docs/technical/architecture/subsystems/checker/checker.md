...
- **The two purge ranges an archive applies are not aligned, and `compareLogs` must account for the overlap.** `processCloseChapter` sets `close_sequence` to the CloseChapter log's **own** sequence but `close_audit_sequence` to the last audit entry written **before** that proposal (`next - 1`). `executePurge` therefore deletes logs `[start, close_sequence]` — the closing proposal's logs included — while the audit entry that produced them, sitting at `close_audit_sequence + 1`, survives; it is in fact the first entry `verifyAuditHashChain` reads, and its success range reaches up to `close_sequence`. Those sequences are consequently **authenticated and purged at the same time**, which drives both directions of the pass:
  - Purged sequences count as accounted for in the authenticated→stored direction. Without that subtraction every archived chapter emits a false `SEQUENCE_GAP` for each log of its closing proposal.
  - The purged-range test runs **before** the authenticated skip in the stored→authenticated direction. A purged range admits no legitimate row whatever authenticates it, so testing authentication first would silently pass over a row that escaped the `DeleteRange` exactly at the boundary — the place residue is most likely to be missed.
