...
**The log iterator's upper bound is the successor of the `[ZoneHistory][SubHistoryLog]` prefix**, not an eight-byte `0xFF` run. Pebble's `UpperBound` is exclusive, so the `0xFF` form — the former `dal.MaxUint64Bytes` — is byte-identical to the key of the row at `math.MaxUint64` and excludes exactly that row: a row planted at the top of the key space would be invisible to the one pass built to see planted rows, the mirror of the reserved sequence 0 hole below. The rule now lives in the key layer as `dal.PrefixUpperBound` / `dal.ZonePrefixUpperBound`, and every sequence-keyed scan in the store uses it — see [the range-bound contract](../storage/range-bounds.md).
