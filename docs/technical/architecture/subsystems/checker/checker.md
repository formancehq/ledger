...
The set of *edges*, however, is load-bearing, and this is the one place the cascade must follow the FSM closely. `GetSigningKeyChildren` returns the **union** of two relations: the revoked key's committed children, minus any the same proposal removed, and the keys that same proposal registered under it. It never consults a reassigned parent pointer to exclude a key. A key re-registered under a new parent *in the same proposal* as a cascade revoke of its old parent is therefore still revoked, so the checker walks both edges: `beginProposal` snapshots the parent relation once per audit entry — one entry is one proposal, matching the FSM's single `WriteSet.Reset` per proposal — and `descendantsOf` treats a key as a child if *either* its snapshotted or its current parent matches. Keys the proposal already removed are absent from the expectation, which reproduces the pending-removal filter for free.

Both halves of that are guards against a false positive in opposite directions. Walking only the running pointer drops the reparented key from the cascade, leaving it expected while the store deleted it. Keeping the snapshot beyond its proposal — including forgetting to refresh it on a revoke in a proposal that registered nothing — cascades from a parent link that has since been committed away, deleting a key that should survive.
...
| `SIGNING_VERIFICATION_INCOMPLETE` | Archived history had to be folded but could not be: no cold reader (restore, or the CLI validating a staged store), a chapter that failed to read, or an order that failed to decode |
