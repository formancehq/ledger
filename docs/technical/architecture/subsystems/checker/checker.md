...
| 10 | `compareMirrorV2LogID` | Stored `LedgerBoundaries.last_mirror_v2_log_id` is `<=` the max audited `MirrorIngest.v2_log_id` per ledger (a **bound**, not equality) | Live audit chain (`recordMirrorIngestMutations`) over a baseline floor (`foldBaselineBoundaries`) | `MIRROR_V2LOGID_AHEAD` (only when stored > audited max) |
...
- **`compareMirrorV2LogID` is a one-sided bound, not an equality.** It flags only `stored > max(audited v2_log_id)` — the data-loss direction, where the EN-1550 idempotency high-water mark claims a source log the audit never recorded and would wrongly skip future ingests. Stored *below* the max (or `0`) is the legitimate legacy / no-upgrade-backfill / self-healing state: existing clusters carry no `last_mirror_v2_log_id` and the field only advances on future ingests, so strict equality would false-positive on every pre-EN-1550 store. The audited max is seeded from a baseline floor so archived-only mirror ingests are not undercounted.
