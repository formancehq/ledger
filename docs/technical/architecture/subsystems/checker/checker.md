...
- **`compareReverseMapOrphans` is the only pass that reads a peer store**, and the only exception to invariant #8's main-store scope. The reverse map is the one read-index limb that cannot be range-deleted by field — its metadata key sits *after* a fixed-width 4-byte version block — so field removal has to scan the namespace and point-delete row by row (`purgeReverseMapForKey`), and a row that scan misses is a permanent divergence no other mechanism detects. The `0x01` forward index and `0x02` existence counters are dropped by a range delete, which is atomic and self-healing.
  ...
  - **Cursor skew is handled per verdict class, not by one gate.** Every oracle term is pinned at the verified log sequence, but the peer read index is folded asynchronously on a live node, so its cursor can sit on either side of that pin. Verdicts resting on *positive evidence* — a replayed `RemovedMetadataFieldType` for the field, a replayed `DeleteLedger` for the ledger — hold at any skew and need only that the peer has folded up to the verified sequence. Verdicts resting on *absence* additionally require the cursor to be exactly aligned, because a field or ledger created after the pin looks identical to a removed one in a view frozen before it. Malformed keys need no oracle and are always reported. `Check()` pins the peer snapshot alongside the primary one to keep the aligned case reachable; the two cannot be taken atomically across two Pebble stores, which is why correctness rests on the class split rather than on the cursors matching.
