...
- **`verifyArchivedChapterResidency` is a residency check, not a projection compare.** It re-derives nothing from the audit chain: the oracle is the registry status plus the ranges recorded on the chapter row, trusted as read — the same trust the replay boundary (`archiveEndSeq`), the cold backfill windows and the signing fold already place in the registry. `verifySealingHash` binds only `chapter_id`, `close_sequence`, `last_audit_hash` and `state_hash`, so a tampered `status`, `start_sequence` or audit range on the row narrows or skips the scan — see the not-verified list below. It runs before the chain walk (it needs only the snapshot and the chapter rows) — including on the zero-log fast path, since a store with no logs can still hold ARCHIVED registry rows with residue in their ranges — and scans exactly the ranges `WriteSet.executePurge` deletes, one event per `(chapter, sub-prefix)` with a resident-key count, so the event stream stays bounded by the chapter count rather than the residue size. Beyond a lost purge, it catches any path that re-ingests archived history into hot storage without purging it afterwards — an incremental restore rebuilt from a cold-backfilled delta is the known case.
...
- **Chapter registry rows beyond the sealing hash**: `verifySealingHash` binds
  `chapter_id`, `close_sequence`, `last_audit_hash` and `state_hash`; the row's
  `status`, `start_sequence`, `start_audit_sequence` and `close_audit_sequence`
  are consumed as read by every archive-aware pass — the replay boundary, the
  cold folds, and the residency scan's ranges. They are not re-derived from a
  chain-bound source: the `ConfirmArchiveChapter` payload carrying them is
  purged from hot storage with its own chapter, so verifying them would need
  the cold fold pattern the signing pass uses. A tampered row can accordingly
  narrow or skip the residency scan, shift the replay boundary, or misdirect a
  cold fold.
