...
| 14 | `compareSchema` | Each live ledger's stored `LedgerInfo.MetadataSchema` field declarations — declared type **and schema revision** — against the audit-derived declarations, both ways (missing, extra, and diverging fields all flag) | `CreateLedger.initial_schema` (the minted schema, revisions included) + replay of `SetMetadataFieldType` / `RemovedMetadataFieldType` logs, revisions taken from each log's mint-time stamp | `SCHEMA_MISMATCH` |
...
- **`compareSchema`'s revision comparison is the detective control for the serving-window gate**: a restore or rebuild that drops the logs' mint-time revisions restarts fields at zero and silently disarms the one-revision stale-binding refusal (see [indexer / indexes.md](../indexer/indexes.md)); revision divergence therefore fails the store check — and the restore service's validate step — as a `SCHEMA_MISMATCH`.
