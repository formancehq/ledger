...
| 14 | `compareSchema` | Each live ledger's stored `LedgerInfo.MetadataSchema` field declarations — declared type, **schema revision, and declaration incarnation** — against the audit-derived declarations, both ways (missing, extra, and diverging fields all flag) | `CreateLedger.initial_schema` (the minted schema, stamps included) + replay of `SetMetadataFieldType` / `RemovedMetadataFieldType` logs, stamps taken from each log's mint-time values | `SCHEMA_MISMATCH` |
...
- **`compareSchema`'s revision and incarnation comparisons are the detective control for the serving-window gate**: a restore or rebuild that drops either mint-time stamp restarts fields at zero and silently disarms one half of the stale-binding refusal — the revision the distance check, the incarnation the cross-lineage check (see [indexer / indexes.md](../indexer/indexes.md)). Divergence in either therefore fails the store check — and the restore service's validate step — as a `SCHEMA_MISMATCH`.
