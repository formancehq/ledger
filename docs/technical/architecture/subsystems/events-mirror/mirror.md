...
- **Coverage pre-declaration**: the worker pre-computes the per-order `plan.Coverage` for the whole batch in one pass (`worker.go:568-630`), so the per-proposal preload work is amortised.
