...
A failed head lookup is logged and does not suppress the batch fetch. A failed
batch is logged, then its safe diagnostic is proposed as `MirrorSyncError`.
The FSM writes that reporting status when the proposal applies; ledger progress
reads expose the same safe message. The ingestion boundary does not advance.
...
userinfo escapes and control characters, including wrapped error chains.
...
- **Audit attribution.** The operator reconstructs `status.appliedIndexes` from singleton creations identified by the CR UID in audit idempotency keys and matched to the current index creation date. Existing external indexes and unrepresentable index kinds are not adopted. Manage operator-created indexes through the spec: manual replacement after verification but before an unconditional drop is an accepted race. See [the attribution contract](../indexer/operator-ownership.md).
