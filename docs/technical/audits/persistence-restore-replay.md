...
| Projection and cascade inventory | Balances/volumes, account metadata and account-wide ephemeral purge signals, transactions/references, LedgerInfo/account types/default enforcement, the monotonic ledger-ID allocator, boundaries/mirror progress, ledger metadata, indexes/registry, numscripts, reversions, idempotency outcomes, signing, policy, maintenance mode, sink config, query checkpoints and their schedule. For account purge, prove checkpoint-era volume and metadata rows are removed by a non-empty post-checkpoint delta while immutable transaction mappings remain. Test create/update/delete/reset variants actually present at the SHA. |
