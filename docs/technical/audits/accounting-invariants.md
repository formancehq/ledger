...
| Numscript equivalence | `processor_transaction_numscript.go`, `numscript_store_adapter_test.go`, `internal/application/admission/numscript*`, `internal/query/numscript*.go`, and scripting docs: trace stored-script version selection, then compare the resolved posting/result semantics with an equivalent direct request. Do not assume identical error text where the documented producer context differs. |
