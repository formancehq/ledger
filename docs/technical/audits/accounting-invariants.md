...
| `integrity-verifier-soundness` | Missing/unsound verification, reconstruction independence, range coverage or loss of checker evidence. If one producer bug also escapes checking, keep the accounting producer finding here only when the checker has a separately demonstrable missing protection with an independent witness. |
