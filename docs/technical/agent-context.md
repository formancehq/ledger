...
`docs/technical/architecture/subsystems/backup/incremental-restore-contract.md`.
...
When authoring or reviewing a change, use the code-area and task-type routing in this document to identify the affected subsystem contracts, then inspect the matching manifests and companion documents under `docs/technical/audits/`. Read the canonical trigger list and decision rules in `docs/technical/contributing/ai-audit.md` before editing an audit contract.

When a canonical maintenance trigger in `docs/technical/contributing/ai-audit.md` fires, the same PR must update both the manifest and its companion document, creating the companion if it is missing. Record no artificial manifest edit when the routed review establishes that the audit contract is unaffected. When a change reveals a new durable, reusable correctness domain, create the manifest and companion for review and merge first; run the audit only in a separate follow-up task against the resulting clean commit.
