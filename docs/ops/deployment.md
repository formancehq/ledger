...
2. **Authentication**: Add API authentication (JWT, OAuth2). `--auth-enabled` with an OIDC issuer requires an explicit `--auth-audience` / `AUTH_AUDIENCE`, identical on all deployment nodes and included in OIDC tokens. Static Ed25519 tokens are exempt and require keys dedicated to the deployment and authentication JWT usage. All authenticated configurations require `--tls-mode=required` and the server refuses to start with `--tls-mode=disabled` or `--tls-mode=optional` — bearer tokens would otherwise be exposed to plaintext interception (`optional` still accepts plaintext client connections). This is intentionally stricter than the `--cluster-secret` rule, which permits `optional` for zero-downtime migration. See [Authentication](authentication.md#configuration-invariants).
