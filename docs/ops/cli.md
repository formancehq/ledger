...
| | | | Bare alphanumeric values are accepted unquoted; values with `-`, `:`, spaces or other non-identifier characters must be single- or double-quoted. Keys containing both `"` and `'` cannot be expressed in the textual DSL and require the gRPC API directly: use `AuditCondition.string_cond` for exact/`in` lookups and `AuditCondition.string_prefix` for prefix lookups. |
