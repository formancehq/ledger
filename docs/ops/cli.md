...
- **HTTP** — `GET /_info` returns flat JSON (no `data` envelope): `{"version":"…","commit":"…","buildDate":"…","goVersion":"…","protocolVersion":"11"}`.
...
Ed25519 tokens generated by `ledgerctl` do not require an audience. Use authentication keys dedicated to this deployment and to signing authentication JWTs.
...
| `--auth-audience` | string | `""` | Required with an OIDC issuer: deployment resource-server identifier (`AUTH_AUDIENCE`), identical on all nodes and matched exactly against OIDC `aud`. Static Ed25519 tokens are exempt |
...
| `--auth-ed25519-keys` | string | `""` | Path to JSON file with Ed25519 public keys and scopes; requires explicit `--auth-enabled` and TLS; use keys dedicated to this deployment and authentication JWTs |
...
When OIDC authentication is enabled, the server performs discovery, downloads the JWKS, and validates JWT signatures, issuer, expiration, and deployment audience on every request. Static Ed25519 tokens do not require an audience; their deployment boundary is the dedicated authentication key set. By default, tokens may use the virtual scopes `ledger:read`, `ledger:write`, and `ledger:admin`, which expand to granular `ledger:<Resource><Action>` scopes.
