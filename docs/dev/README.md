# Developer Guide

Everything you need to contribute to the Ledger v3 project.

## I want to...

| Goal | Document |
|------|----------|
| Get started on the project | [getting-started.md](./getting-started.md) |
| Understand code conventions | [conventions.md](./conventions.md) |
| Write and run tests | [testing.md](./testing.md) |
| Work with Protocol Buffers | [protobuf.md](./protobuf.md) |
| Understand Numscript | [numscript.md](./numscript.md) |
| Track v2 API parity | [api-comparison.md](./api-comparison.md) |
| Explore the architecture | [architecture/](./architecture/) |

## Architecture Deep-Dives

| Topic | Document |
|-------|----------|
| System overview (start here) | [architecture/overview.md](./architecture/overview.md) |
| Raft consensus | [architecture/raft-consensus.md](./architecture/raft-consensus.md) |
| Deterministic FSM | [architecture/deterministic-fsm.md](./architecture/deterministic-fsm.md) |
| Storage engine | [architecture/storage.md](./architecture/storage.md) |
| Data flows | [architecture/data-flows.md](./architecture/data-flows.md) |
| Global log | [architecture/global-log.md](./architecture/global-log.md) |
| gRPC API | [architecture/grpc-api.md](./architecture/grpc-api.md) |
| HTTP API | [architecture/api.md](./architecture/api.md) |
