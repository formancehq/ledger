...
func NewCreateCommand() *cobra.Command {
	...
	cmd.Flags().String("mirror-source-type", "ledgerV2Http", "Mirror source type: ledgerV2Http or ledgerV2Database")
	cmdutil.RegisterEnumCompletion(cmd, "mirror-source-type", "ledgerV2Http", "ledgerV2Database")
	cmd.Flags().String("mirror-ledger-name", "", "Source ledger name in the v2 system (defaults to ledger name)")
	cmd.Flags().String("mirror-base-url", "", "Base URL of the v2 API (for ledgerV2Http source)")
	cmd.Flags().String("mirror-oauth2-client-id", "", "OAuth2 client ID for the v2 API (for ledgerV2Http source)")
	cmd.Flags().String("mirror-oauth2-client-secret", "", "OAuth2 client secret for the v2 API (for ledgerV2Http source)")
	cmd.Flags().String("mirror-oauth2-token-endpoint", "", "OAuth2 token endpoint URL (for ledgerV2Http source)")
	cmd.Flags().StringArray("mirror-oauth2-scopes", nil, "OAuth2 scopes (for ledgerV2Http source, can be repeated)")
	cmd.Flags().String("mirror-dsn", "", "PostgreSQL DSN (for ledgerV2Database source)")
	cmd.Flags().String("mirror-aws-iam-region", "", "Enable AWS RDS IAM authentication using the given region (for ledgerV2Database source); credentials are taken from the ambient AWS chain (IRSA, instance profile, env)")
