...
func runStoreMetrics(cmd *cobra.Command, _ []string) error {
	...
	if err != nil {
		...
		return formatGRPCError("failed to get store metrics", err)
	}
...
func printFormattedMetrics(m *servicepb.PebbleMetrics) {
	...
	if m.BlockCache != nil {
		...
		tableData := pterm.TableData{
			{"Metric", "Value"},
			{"Size", formatBytes(uint64(m.BlockCache.Size))},
	...
	if m.MemTable != nil {
		...
		tableData := pterm.TableData{
			{"Metric", "Value"},
			{"Size", formatBytes(m.MemTable.Size)},
			{"Count", fmt.Sprintf("%d", m.MemTable.Count)},
			{"Zombie Size", formatBytes(m.MemTable.ZombieSize)},
	...
	if m.Wal != nil {
		...
		tableData := pterm.TableData{
			...
			{"Size", formatBytes(m.Wal.Size)},
			{"Bytes In", formatBytes(m.Wal.BytesIn)},
			{"Bytes Written", formatBytes(m.Wal.BytesWritten)},
		}
		_ = pterm.DefaultTable.WithHasHeader().WithData(tableData).Render()
		pterm.Println()
	}
	...
	if m.Compact != nil {
		...
		tableData := pterm.TableData{
			...
			{"Estimated Debt", formatBytes(m.Compact.EstimatedDebt)},
			{"In Progress Bytes", formatBytes(uint64(m.Compact.InProgressBytes))},
	...
	if m.Flush != nil {
		...
		tableData := pterm.TableData{
			...
			{"As Ingest Bytes", formatBytes(m.Flush.AsIngestBytes)},
		}
		_ = pterm.DefaultTable.WithHasHeader().WithData(tableData).Render()
		pterm.Println()
	}
	...
	if m.Snapshots != nil {
		...
		tableData := pterm.TableData{
			...
			{"Pinned Size", formatBytes(m.Snapshots.PinnedSize)},
		}
		_ = pterm.DefaultTable.WithHasHeader().WithData(tableData).Render()
		pterm.Println()
	}
	...
	if len(m.Levels) > 0 {
		...
		for _, level := range m.Levels {
			tableData = append(tableData, []string{
				...
				formatBytes(uint64(level.Size)),
				fmt.Sprintf("%.2f", level.Score),
				formatBytes(level.BytesIn),
				formatBytes(level.BytesCompacted),
			})
		}
		_ = pterm.DefaultTable.WithHasHeader().WithData(tableData).Render()
	}
