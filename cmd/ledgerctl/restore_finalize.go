...
// newRestoreFinalizeCommand creates the restore finalize command.
func newRestoreFinalizeCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "finalize",
...
func runRestoreFinalize(cmd *cobra.Command, _ []string) error {
	...
	if err != nil {
		return formatGRPCError("failed to finalize restore", err)
	}
