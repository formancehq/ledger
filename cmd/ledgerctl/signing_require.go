...
// newSigningRequireCommand creates the signing require command.
func newSigningRequireCommand() *cobra.Command {
	cmd := &cobra.Command{
...
func runSigningRequire(cmd *cobra.Command, args []string) error {
	var require bool
	switch args[0] {
	...
	if err != nil {
		...
		return formatGRPCError("failed to update signing config", err)
	}
