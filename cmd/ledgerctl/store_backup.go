...
// newStoreBackupCommand creates the store backup command.
func newStoreBackupCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "backup",
		Aliases: []string{"bk"},
...
func runStoreBackup(cmd *cobra.Command, _ []string) error {
	...
	if err != nil {
		return formatGRPCError("failed to start backup", err)
	}
	...
	for {
		...
		if err != nil {
			...
			return formatGRPCError("receiving backup chunk", err)
		}
		...
		if len(resp.Data) > 0 {
			...
			if spinner != nil {
				spinner.UpdateText(fmt.Sprintf("Downloading backup... %s", formatBytes(totalReceived)))
			}
		}
	}
	...
	if outputPath != "" {
		pterm.Success.Printfln("Backup saved to %s (%s, SHA256: %s)",
			pterm.Cyan(outputPath), formatBytes(expectedSize), actualHash[:12]+"...")
	}
