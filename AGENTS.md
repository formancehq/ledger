...
8. **The checker verifies the authoritative store, not every derived projection** — Only `AuditEntry` (zone `Cold`, sub `Audit`) is cryptographically bound: the hash chain that `state.BuildHashedHeaderPayload` + `processing.HashGenerator` produce and `checker.verifyAuditHashChain` verifies on every `Check()`. Everything else in Pebble is a projection of orders that already live in that chain. Classify new persisted data by **which store holds it**:

   - **Primary FSM store** (the single store `Check()` opens and walks) — *in scope*. If the datum is authoritative, security-sensitive, or can affect ledger business correctness independently of the store it derives from, `internal/application/check/checker.go` MUST re-derive its expected value by replaying the audit (or another already-verified source) and compare it to what is stored, emitting the matching `CHECK_STORE_ERROR_TYPE_*` on divergence. A projection is exempt as merely derived only when it is deterministically rebuildable from still-retained verified state through a real, wired rebuild path — not rebuildable "in principle".
   - **Peer secondary store** (`usagestore` counters, `readstore` index) — *out of scope by construction*: `Check()` never opens them. They are pure rebuildable caches of the audit chain, so corruption is a *rebuild* event, not an undetected integrity violation. Serving a value through an API does not make it authoritative (API-visibility ≠ authoritativeness).

   Replication/ingestion cursors are technical state, not business invariants: the `MirrorCursor` pointer is only the worker's progress byte. But the correctness-bearing high-water mark `LedgerBoundaries.last_mirror_v2_log_id` lives in the primary store, so it IS verified (`compareMirrorV2LogID`, EN-1550) — the checker guards the applied prefix, not the pointer. See [audit-vs-technical-state.md](docs/technical/architecture/audit-vs-technical-state.md) for the full mirror-cursor reasoning.

   Current passes: `compareVolumes`, `compareMetadata`, `compareTransactions`, `compareExclusionProjections` (AppliedProposal.TransientVolumes + LedgerLog.PurgedVolumes), `checkReversionInvariants`, `verifySealingHash`, `compareIdempotencyOutcomes` (frozen SubIdempKeys outcomes vs the hash-chained AuditFailure/AuditSuccess — the failure kind is re-derived from the chain-bound reason via `domain.KindForReason`, never stored), `compareIndexes` (SubAttrIndex registry vs CreateIndex/DropIndex/RemovedMetadataFieldType/DeleteLedger logs — presence + identity; BuildStatus excluded as informational), and `compareMirrorV2LogID` (`last_mirror_v2_log_id` **==** `max(audited MirrorIngest.v2_log_id)` per ledger). Extend the list as new authoritative persisted state lands.
