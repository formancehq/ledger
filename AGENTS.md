...
Audit manifests are living correctness contracts. If a change alters a covered surface, invariant, input, exclusion, evidence oracle, or adversarial scenario, update the manifest and its companion document in the same PR; do not force churn when the contract is unaffected. Authors and reviewers must use the audit-contract routing in `docs/technical/agent-context.md`. A new reusable domain follows manifest review and merge first, then audit execution in a separate task.
