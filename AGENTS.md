...
Audit manifests are living correctness contracts. If a change alters a covered surface, invariant, input, exclusion, evidence oracle, or adversarial scenario, update the manifest and its companion document in the same PR; do not force churn when the contract is unaffected. Authors and reviewers must use the audit-contract routing in `docs/technical/agent-context.md`; the canonical trigger list and decision rules live in `docs/technical/contributing/ai-audit.md`. A new reusable domain follows manifest review and merge first, then audit execution in a separate task.
...
- `docs/technical/architecture/subsystems/backup/incremental-restore-contract.md` when the projection changes during audited apply or another lifecycle path covered by incremental backup
