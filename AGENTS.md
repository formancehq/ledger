...
Audit manifests are living correctness contracts. When a canonical maintenance trigger in `docs/technical/contributing/ai-audit.md` fires, update the manifest and its companion document in the same PR; create the companion if it is missing, but do not force churn when the contract is unaffected. Authors and reviewers must use the audit-contract routing in `docs/technical/agent-context.md`. A new reusable domain follows manifest review and merge first, then audit execution in a separate task.
