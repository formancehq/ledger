	...
	case balanceRegex.MatchString(property) || property == "balance":
		...
		if opts.PIT != nil && !opts.PIT.IsZero() {
			...
			selectBalance = selectBalance.
				...
				DistinctOn("asset").
				ColumnExpr("first_value((post_commit_effective_volumes).inputs - (post_commit_effective_volumes).outputs) over (partition by (accounts_address, asset) order by effective_date desc) as balance").
