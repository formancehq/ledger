...
func (store *Store) buildVolumesWithBalancesQuery(query *bun.SelectQuery, q ledgercontroller.GetVolumesWithBalancesQuery, where string, args []any, useMetadata bool) *bun.SelectQuery {
	...
	selectAccounts := store.GetDB().NewSelect().
		...
		Group("ledger", "accounts_seq", "accounts_address", "accounts_address_array", "asset").
	...
	if filtersForVolumes.GroupLvl > 0 {
		globalQuery = globalQuery.
			ColumnExpr(fmt.Sprintf(`(array_to_string((string_to_array(accounts_address, ':'))[1:LEAST(array_length(string_to_array(accounts_address, ':'),1),%d)],':')) as account`, filtersForVolumes.GroupLvl)).
			ColumnExpr("asset").
	...
	globalQuery = globalQuery.Order("account", "asset")
