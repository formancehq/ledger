...
// --- EnsureCommitDurable / durable commit watermark tests ---

// Append with entries triggers MustSync inside etcd's WAL, so the commit
// becomes durable synchronously and EnsureCommitDurable returns immediately.
func TestEnsureCommitDurable_EntryAppendIsSynchronous(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, uint64(3), w.durableCommit.Load(),
		"durableCommit must advance synchronously when MustSync fires")

	done := make(chan struct{})
	go func() {
		w.EnsureCommitDurable(3)
		close(done)
	}()
	select {
	case <-done:
	case <-time.After(time.Second):
		t.Fatal("EnsureCommitDurable did not return immediately for already-durable commit")
	}
}

// Commit-only Append (no entries, same term/vote) is the path etcd's WAL
// doesn't fsync. The async syncer must eventually catch up so the watermark
// advances.
func TestEnsureCommitDurable_CommitOnlyAppendCatchesUp(t *testing.T) {
	t.Parallel()

	w := newTestWAL(t)

	// Seed entries so the next Append can be commit-only.
	...
	// Commit-only Append: same term/vote, no entries. MustSync == false.
	err := w.Append(raftpb.HardState{Term: 1, Vote: 1, Commit: 3}, nil)
	require.NoError(t, err)

	// Watermark must catch up via the syncer (bounded wait).
	require.Eventually(t, func() bool {
		return w.durableCommit.Load() >= 3
	}, time.Second, 5*time.Millisecond, "durableCommit must catch up via async syncer")
}

// EnsureCommitDurable blocks the caller until the syncer reaches the target.
// We exercise it by issuing a commit-only Append and immediately calling
// EnsureCommitDurable on the same commit value.
func TestEnsureCommitDurable_BlocksUntilDurable(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, uint64(1), w.durableCommit.Load())

	// Bump commit only.
	require.NoError(t, w.Append(raftpb.HardState{Term: 1, Vote: 1, Commit: 5}, nil))

	done := make(chan struct{})
	go func() {
		w.EnsureCommitDurable(5)
		close(done)
	}()
	...
	case <-done:
	case <-time.After(time.Second):
		t.Fatal("EnsureCommitDurable never returned for target=5")
	}

	require.GreaterOrEqual(t, w.durableCommit.Load(), uint64(5))
}

// Watermark must be seeded from the on-disk HardState on startup, so a
// fresh WAL replay doesn't drop EnsureCommitDurable callers into an
// unnecessary fsync.
func TestEnsureCommitDurable_SeededFromReplay(t *testing.T) {
	t.Parallel()
	...
	w1, err := New(dir, logger, meter)
	require.NoError(t, err)

	require.NoError(t, w1.Append(
		raftpb.HardState{Term: 2, Vote: 1, Commit: 7},
		[]raftpb.Entry{
			{Index: 1, Term: 1, Data: []byte("a")},
			{Index: 2, Term: 1, Data: []byte("b")},
			{Index: 3, Term: 1, Data: []byte("c")},
	...
	// Reopen and confirm the watermark reflects the on-disk HardState.
	w2, err := New(dir, logger, meter)
	require.NoError(t, err)
	t.Cleanup(func() { _ = w2.Close() })

	require.Equal(t, uint64(7), w2.durableCommit.Load())
}
