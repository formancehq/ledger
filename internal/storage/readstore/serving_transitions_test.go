...
// failure FlushServingTransitions exists to close.
...
func TestServingTransition_FlushedPromotionSurvivesAKill(t *testing.T) {
	...
	s.MarkServingTransition(servingLedger, servingCanonical)
	commitVersionState(t, s, IndexVersionState{CurrentVersion: 2, HighWater: 2})
	require.NoError(t, s.FlushServingTransitions())
	...
	assert.False(t, reopened.ServingTransitionInFlight(servingLedger, servingCanonical), "marks are in-memory; a reopened store has nothing in flight")
}
...
func TestServingTransition_InFlightReadsAsBuildingUntilFlushed(t *testing.T) {
	...
	require.NoError(t, s.FlushServingTransitions())
	assert.False(t, s.ServingTransitionInFlight(servingLedger, servingCanonical))
...
func TestServingTransition_UnmarkWithdrawsACancelledPromotion(t *testing.T) {
	...
	s.MarkServingTransition(servingLedger, servingCanonical)
	s.UnmarkServingTransition(servingLedger, servingCanonical)
...
func TestFlushServingTransitions_FailureKeepsTheMarks(t *testing.T) {
	t.Parallel()
	...
	s.OverrideServingFlushForTest(func() error { return errors.New("disk on fire") })
	require.Error(t, s.FlushServingTransitions())
	assert.True(t, s.ServingTransitionInFlight(servingLedger, servingCanonical))
	...
	s.OverrideServingFlushForTest(nil)
	require.NoError(t, s.FlushServingTransitions())
	assert.False(t, s.ServingTransitionInFlight(servingLedger, servingCanonical))
}
...
func TestFlushServingTransitions_FlushesOnlyWhenMarked(t *testing.T) {
	t.Parallel()
