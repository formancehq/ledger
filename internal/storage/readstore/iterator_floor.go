...
// an ascending Seek(t) proves no entity >= t exists in the iterator's view. The proof is
...
// seeks per merge step, and the composite Seek child loops (AND, OR,
...
// covers reports whether an ascending Seek(target) is proven empty by a prior failure.
func (f *seekFloor) covers(target []byte) bool {
	return f.set && bytes.Compare(target, f.bound) >= 0
}

// fail records a failed ascending Seek(target): no entity >= target exists. err is the
...
// seekCeil is the reverse-iteration analog of seekFloor: a failed descending Seek(t)
...
// covers reports whether a descending Seek(target) is proven empty by a prior failure.
func (c *seekCeil) covers(target []byte) bool {
	return c.set && bytes.Compare(target, c.bound) <= 0
}

// fail records a failed descending Seek(target): no entity <= target exists. A non-nil
