...
func TestCompile_StaleBindingReadsAsBuilding(t *testing.T) {
	...
	// Third declaration of tier within one lineage: INT32 → STRING → INT64.
	schema := map[string]*commonpb.MetadataFieldSchema{
		"tier": {Type: commonpb.MetadataType_METADATA_TYPE_INT64, Revision: 3, Incarnation: 7},
	}
	...
	// Bindings default to the schema's lineage; incarnationResolver drives the
	// cross-incarnation cases.
	resolver := func(rev uint32, typ commonpb.MetadataType, declared bool) readstore.IndexVersionResolver {
		return incarnationResolver(rev, 7, typ, declared)
	}
	...
	t.Run("a dropped incarnation's binding is refused, not served", func(t *testing.T) {
		t.Parallel()

		// Removal deletes the declaration, so a re-declaration restarts the
		// revision at 1 while a rewound read store can still hold the dropped
		// incarnation's binding at a HIGHER revision. Revision distance reads
		// that as "comfortably inside the window" (2 + 1 >= 1) and would serve
		// a keyspace the removal purged. The incarnation check is what refuses
		// it, and it needs no fold alignment to do so: the two lineage ids come
		// from two stores, and any skew between the samples can only make them
		// differ, which refuses.
		redeclared := map[string]*commonpb.MetadataFieldSchema{
			"tier": {Type: commonpb.MetadataType_METADATA_TYPE_STRING, Revision: 1, Incarnation: 41},
		}
		...
		_, err := Compile(
			...
			incarnationResolver(2, 10, commonpb.MetadataType_METADATA_TYPE_STRING, true), nil, nil, 0)
		requireBuilding(t, err)
	})

	t.Run("a binding one revision ahead of the schema still serves", func(t *testing.T) {
		t.Parallel()

		// The atomic switch can land between the schema read and the state
		// read, so a healthy binding is legitimately one revision ahead within
		// its own lineage. Refusing "ahead" outright would reject this.
		olderSchema := map[string]*commonpb.MetadataFieldSchema{
			"tier": {Type: commonpb.MetadataType_METADATA_TYPE_INT64, Revision: 2, Incarnation: 7},
		}
		...
		iter, err := Compile(
			...
			commonpb.QueryTarget_QUERY_TARGET_ACCOUNTS, ledgerName,
			nil, olderSchema, info, indexRegistry,
			incarnationResolver(3, 7, commonpb.MetadataType_METADATA_TYPE_INT64, true), nil, nil, 0)
		require.NoError(t, err)
...
// incarnationResolver builds a version resolver with an explicit declaration
// lineage, so a test can put the binding in a different incarnation from the
// schema it is judged against.
func incarnationResolver(rev uint32, incarnation uint64, typ commonpb.MetadataType, declared bool) readstore.IndexVersionResolver {
