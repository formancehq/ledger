...
// through the handle after the projection snapshot is open (requireLedgerLive).
func AlignedIndexSnapshot(ctx context.Context, rs *readstore.Store, mainReader *dal.ReadHandle, ledgerName string, releaseHold func()) (*pebble.Snapshot, uint64, func(), error) {
	...
	for {
		...
		if lastIndexed >= mainAppliedIndex {
			if err := requireLedgerLive(ctx, mainReader, ledgerName); err != nil {
				_ = snap.Close()
				releaseLease()
...
// empty universe is indistinguishable from a wiped one. The projection
// snapshot is already open when this runs, and the main snapshot taken here
// is strictly later. The builder folds a deletion only after the main store
// applied it, so a projection snapshot holding the wipe is always followed
// by a main snapshot showing the ledger deleted; a ledger live here proves
// the projection snapshot still holds every row of the pinned ledger. The
// FSM rejects creating a ledger under a soft-deleted name, so a live ledger
// is the pinned one. A deletion applied after the projection snapshot is
// rejected too, although that snapshot could have been served: NotFound is
// the answer every later read gives, so the read stays linearizable.
//
// The snapshot comes from the handle, which already holds the store's
// lifecycle lock: see dal.ReadHandle.Latest.
func requireLedgerLive(ctx context.Context, mainReader *dal.ReadHandle, ledgerName string) error {
	live := mainReader.Latest()
	// Snapshot.Close only unlinks the snapshot; it cannot fail.
	defer func() { _ = live.Close() }()

	if _, err := GetLedgerByName(ctx, live, ledgerName); err != nil {
