...
func TestAttributeReplayWriter_SchemaOpForUnknownLedgerFailsLoudly(t *testing.T) {
	...
	setErr := writer.SetMetadataFieldType("ghost", commonpb.TargetType_TARGET_TYPE_ACCOUNT, "k",
		&commonpb.MetadataFieldSchema{Type: commonpb.MetadataType_METADATA_TYPE_STRING, Revision: 1, Incarnation: 1})
...
func TestRebuildDelta_BumpsCheckpointIndexOnRetype(t *testing.T) {
	...
	require.Equal(t, uint64(41), field.GetIncarnation(), "and the stamped declaration lineage — dropping it disarms the cross-incarnation gate")
}
...
func TestAttributeReplayWriter_SetMetadataFieldType_LedgerTarget(t *testing.T) {
	...
	require.NoError(t, writer.SetMetadataFieldType("ledger", commonpb.TargetType_TARGET_TYPE_TRANSACTION, "batch_id",
		&commonpb.MetadataFieldSchema{Type: commonpb.MetadataType_METADATA_TYPE_UINT64, Revision: 3, Incarnation: 1}))
	require.NoError(t, writer.batch.Commit())
...
func TestAttributeReplayWriter_RetypeCascade_MissingRowIsNoOp(t *testing.T) {
	...
	require.NoError(t, writer.SetMetadataFieldType("ledger", commonpb.TargetType_TARGET_TYPE_ACCOUNT, "tier",
		&commonpb.MetadataFieldSchema{Type: commonpb.MetadataType_METADATA_TYPE_INT64, Revision: 1, Incarnation: 1}))
	require.NoError(t, writer.batch.Commit())
...
func TestAttributeReplayWriter_RetypeCascade_TombstoneShadowsCheckpointRow(t *testing.T) {
	...
	require.NoError(t, writer.SetMetadataFieldType("ledger", commonpb.TargetType_TARGET_TYPE_ACCOUNT, "tier",
		&commonpb.MetadataFieldSchema{Type: commonpb.MetadataType_METADATA_TYPE_UINT64, Revision: 2, Incarnation: 1}))
	require.NoError(t, writer.batch.Commit())
