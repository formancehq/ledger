...
func newTestMachineWithThreshold(t *testing.T, generationThreshold uint64) (*Machine, *dal.Store, *attributes.Attributes) {
	...
	machine, err := NewMachine(logger, dataStore, noopCommitBarrier{}, meter, c, attrs, keystore.NewKeyStore(), NewSharedState(), noopNotifier{}, noopNotifier{}, noopNotifier{}, nil, 0, false, 0)
	require.NoError(t, err)
