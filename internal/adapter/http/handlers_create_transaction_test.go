...
// TestHandleCreateTransaction_OrderSkipped exercises the 200/OrderSkipped path
// the handler returns when the FSM converts a whitelisted failure into an
// OrderSkippedLog. The skipped: true flag is the canonical branch point for
// clients; the reason and context surface the underlying business outcome
// (here the existing transaction id of the prior reference holder).
