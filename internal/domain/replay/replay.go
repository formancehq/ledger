...
func ReplayLedgerLog(
	...
	case *commonpb.LedgerLogPayload_SetMetadataFieldType:
		if l := p.SetMetadataFieldType; l != nil {
			...
			if err := w.SetMetadataFieldType(ledger, l.GetTargetType(), l.GetKey(), field); err != nil {
...
func replayEphemeralPurge(
	...
	// that need derived exclusions go through the buffered path which flushes
...
// sequence already covered by the replay base, such as a backup checkpoint.
	...
	for _, posting := range postings {
		for _, addr := range []string{posting.GetSource(), posting.GetDestination()} {
			...
			for _, p := range postings {
				...
				// Test the cumulative balance the Writer holds.
