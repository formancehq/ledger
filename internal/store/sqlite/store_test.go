...
func testStoreCommon(t *testing.T, createStore func(*testing.T) store.Store) {
	...
	t.Run("BalancesCalculation", func(t *testing.T) {
		...
		registerLedger(ctx, t, s, "test-ledger", testLedgerID)
		batch := s.NewBatch(0)
		require.NoError(t, batch.AppendBalanceDiff(ctx, testLedgerID, "world", "USD", big.NewInt(-100)))
		require.NoError(t, batch.AppendBalanceDiff(ctx, testLedgerID, "bank", "USD", big.NewInt(100)))
		require.NoError(t, batch.AppendBalanceDiff(ctx, testLedgerID, "user", "USD", big.NewInt(50)))
		require.NoError(t, batch.AppendBalanceDiff(ctx, testLedgerID, "bank", "USD", big.NewInt(-50)))
