...
type StoreInterceptor struct {
	...
	OnGetAllLogs       func(delegate *Store, from, to uint64) (store.Cursor[*commonpb.Log], error)
	OnGetLogBySequence func(delegate *Store, sequence uint64) (*commonpb.Log, error)

	// Store interceptors
	OnListLedgers                  func(delegate *Store) (store.Cursor[*commonpb.LedgerInfo], error)
	OnGetBalanceDiffs              func(delegate *Store, ledgerID uint32, query store.BalanceDiffsQuery) (store.BalanceDiffsResult, error)
	OnGetBalanceBase               func(delegate *Store, ledgerID uint32, account, asset string, maxRaftIndex uint64) (*store.StoredBalanceBase, error)
	...
	OnNewBatch                     func(delegate *Store, lastAppliedIndex uint64) store.Batch
	...
	BatchInterceptorFactory func(batch store.Batch) store.Batch
}
...
func (s *StoreInterceptor) GetAllLogs(from, to uint64) (store.Cursor[*commonpb.Log], error) {
	s.mu.RLock()
...
func (s *StoreInterceptor) ListLedgers() (store.Cursor[*commonpb.LedgerInfo], error) {
	s.mu.RLock()
...
func (s *StoreInterceptor) GetBalanceDiffs(ledgerID uint32, query store.BalanceDiffsQuery) (store.BalanceDiffsResult, error) {
	s.mu.RLock()
...
func (s *StoreInterceptor) GetBalanceBase(ledgerID uint32, account, asset string, maxRaftIndex uint64) (*store.StoredBalanceBase, error) {
	s.mu.RLock()
...
func (s *StoreInterceptor) NewBatch(lastAppliedIndex uint64) store.Batch {
	s.mu.RLock()
...
func (s *StoreInterceptor) SetGetAllLogsInterceptor(fn func(delegate *Store, from, to uint64) (store.Cursor[*commonpb.Log], error)) {
	s.mu.Lock()
	s.OnGetAllLogs = fn
	s.mu.Unlock()
}
...
func (s *StoreInterceptor) SetListLedgersInterceptor(fn func(delegate *Store) (store.Cursor[*commonpb.LedgerInfo], error)) {
	s.mu.Lock()
	s.OnListLedgers = fn
	s.mu.Unlock()
}

func (s *StoreInterceptor) SetGetBalanceDiffsInterceptor(fn func(delegate *Store, ledgerID uint32, query store.BalanceDiffsQuery) (store.BalanceDiffsResult, error)) {
	s.mu.Lock()
	s.OnGetBalanceDiffs = fn
	s.mu.Unlock()
}

func (s *StoreInterceptor) SetNewBatchInterceptor(fn func(delegate *Store, lastAppliedIndex uint64) store.Batch) {
	s.mu.Lock()
	s.OnNewBatch = fn
	s.mu.Unlock()
}

func (s *StoreInterceptor) SetBatchInterceptorFactory(fn func(batch store.Batch) store.Batch) {
	s.mu.Lock()
