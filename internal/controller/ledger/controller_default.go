...
func (ctrl *DefaultController) BeginTX(ctx context.Context, options *sql.TxOptions) error {
	return ctrl.store.BeginTX(ctx, options)
}
...
func (ctrl *DefaultController) Import(ctx context.Context, stream chan ledger.Log) error {
	...
	err := ctrl.store.BeginTX(ctx, &sql.TxOptions{Isolation: sql.LevelSerializable})
	if err != nil {
		return fmt.Errorf("failed to start transaction: %w", err)
	}
	defer func() {
		_ = ctrl.store.Rollback()
	}()
	...
	if err := ctrl.store.LockLedger(ctx); err != nil {
		return fmt.Errorf("failed to lock ledger: %w", err)
	}

	// We can import only if the ledger is empty.
	logs, err := ctrl.store.ListLogs(ctx, NewListLogsQuery(PaginatedQueryOptions[any]{
		PageSize: 1,
	}))
	if err != nil {
	...
	if err := ctrl.store.Commit(); err != nil {
...
func (ctrl *DefaultController) createTransaction(ctx context.Context, sqlTX Store, parameters Parameters[RunScript]) (*ledger.CreatedTransaction, error) {
