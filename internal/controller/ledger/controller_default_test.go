...
func TestGetAggregatedBalances(t *testing.T) {
	...
	aggregatedBalances.EXPECT().GetOne(gomock.Any(), pagination.ResourceQuery[GetAggregatedVolumesOptions]{}).
		Return(&ledger.AggregatedVolumes{}, nil)

	l := NewDefaultController(ledger.Ledger{}, store, parser)
	ret, err := l.GetAggregatedBalances(ctx, pagination.ResourceQuery[GetAggregatedVolumesOptions]{})
	require.NoError(t, err)
...
func TestGetVolumesWithBalances(t *testing.T) {
	...
	parser := NewMockNumscriptParser(ctrl)
	ctx := logging.TestingContext()
	volumes := NewMockPaginatedResource[ledger.VolumesWithBalanceByAssetByAccount, GetVolumesOptions, pagination.OffsetPaginatedQuery[GetVolumesOptions]](ctrl)

	balancesByAssets := &bunpaginate.Cursor[ledger.VolumesWithBalanceByAssetByAccount]{}
	store.EXPECT().Volumes().Return(volumes)
	volumes.EXPECT().Paginate(gomock.Any(), pagination.OffsetPaginatedQuery[GetVolumesOptions]{
		PageSize: bunpaginate.QueryDefaultPageSize,
		Order:    pointer.For(bunpaginate.Order(bunpaginate.OrderAsc)),
	}).Return(balancesByAssets, nil)

	l := NewDefaultController(ledger.Ledger{}, store, parser)
	ret, err := l.GetVolumesWithBalances(ctx, pagination.OffsetPaginatedQuery[GetVolumesOptions]{
		PageSize: bunpaginate.QueryDefaultPageSize,
		Order:    pointer.For(bunpaginate.Order(bunpaginate.OrderAsc)),
	})
	require.NoError(t, err)
