...
// createTestLogsWithPrefix creates test logs with a prefix for idempotency keys
func createTestLogsWithPrefix(t *testing.T, ledgerName string, prefix string) []ledger.Log {
	now := libtime.New(time.Now())
	...
	logs := []ledger.Log{
		...
		ledger.NewLog(&ledger.CreatedTransaction{
			...
			WithLedger(ledgerName).
			WithID(1).
			WithIdempotencyKey(prefix + "idempotency-key-1").
			WithDate(now),
		...
		ledger.NewLog(&ledger.CreatedTransaction{
			...
			WithLedger(ledgerName).
			WithID(2).
			WithIdempotencyKey(prefix + "idempotency-key-2").
			WithDate(now.Add(time.Second)),
