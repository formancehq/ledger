...
func TestReadLogBySequence(t *testing.T) {
	...
	log, err := state.ReadLogBySequence(s, 1)
	require.NoError(t, err)
	require.NotNil(t, log)
	require.Equal(t, uint64(1), log.Sequence)

	log, err = state.ReadLogBySequence(s, 999)
	require.NoError(t, err)
	require.Nil(t, log)
}
...
func TestReadLastSequence(t *testing.T) {
	...
	lastSequence, err := state.ReadLastSequence(s)
	require.NoError(t, err)
...
func TestGetLedgerByName(t *testing.T) {
	...
	ledger, err := state.GetLedgerByName(s, "my-ledger")
	require.NoError(t, err)
	require.NotNil(t, ledger)
	require.Equal(t, "my-ledger", ledger.Name)
	require.Equal(t, uint32(42), ledger.Id)

	ledger, err = state.GetLedgerByName(s, "non-existing")
	require.Error(t, err)
...
func TestReadLastAppliedIndex(t *testing.T) {
	...
	lastIndex, err := state.ReadLastAppliedIndex(s)
	require.NoError(t, err)
...
func TestReadLedgersSoftDelete(t *testing.T) {
	...
	require.NoError(t, state.StoreTransactionUpdate(batch, dal.TransactionKey{LedgerID: ledgerID, ID: 1}, &commonpb.TransactionUpdate{
		ByLog: 1,
...
func TestReadPeriods(t *testing.T) {
	...
	t.Run("EmptyStore", func(t *testing.T) {
		...
		periods, err := state.ReadAllPeriods(s)
		require.NoError(t, err)
		require.Nil(t, periods)

		nextID, err := state.ReadNextPeriodID(s)
		require.NoError(t, err)
	...
	t.Run("PersistAcrossReopen", func(t *testing.T) {
		...
		periods, err := state.ReadAllPeriods(s2)
		require.NoError(t, err)
		require.Len(t, periods, 2)
		require.Equal(t, uint64(1), periods[0].Id)
		require.Equal(t, uint64(2), periods[1].Id)

		nextID, err := state.ReadNextPeriodID(s2)
		require.NoError(t, err)
...
func TestReadSigningKeys(t *testing.T) {
	...
	t.Run("empty store has no signing keys", func(t *testing.T) {
		keys, err := state.ReadSigningKeys(s)
		require.NoError(t, err)
		require.Empty(t, keys)

		requireSig, err := state.ReadSigningConfig(s)
		require.NoError(t, err)
...
func TestReadLogsSince(t *testing.T) {
	...
	t.Run("EmptyStore", func(t *testing.T) {
		...
		cursor, err := state.ReadLogsSince(s, 0)
		require.NoError(t, err)
	...
	t.Run("LogsAfterSequence", func(t *testing.T) {
		...
		cursor, err := state.ReadLogsSince(s, 2)
		require.NoError(t, err)
		logs := collectLogs(t, cursor)
	...
	t.Run("LogsAfterLastSequence", func(t *testing.T) {
		...
		cursor, err := state.ReadLogsSince(s, 4)
		require.NoError(t, err)
		logs := collectLogs(t, cursor)
		require.Empty(t, logs)
	})
	...
	t.Run("LogsAfterFarFutureSequence", func(t *testing.T) {
		...
		cursor, err := state.ReadLogsSince(s, 999)
		require.NoError(t, err)
		logs := collectLogs(t, cursor)
		require.Empty(t, logs)
	})
	...
	t.Run("IncrementalRead", func(t *testing.T) {
		...
		// Read only new logs
		cursor, err = state.ReadLogsSince(s, lastSeq)
		require.NoError(t, err)
