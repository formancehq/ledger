...
func (s *PostgresLogStore) createTables(ctx context.Context) error {
	...
	// Add sequence column if it doesn't exist (for migration)
	// Note: sequence is provided by the FSM, not auto-generated by the database
	_, _ = s.db.ExecContext(ctx, `
		ALTER TABLE logs ADD COLUMN IF NOT EXISTS sequence BIGINT NOT NULL DEFAULT 0;
...
func (s *PostgresLogStore) scanLog(row *sql.Row) (*ledger.Log, error) {
	...
	err := row.Scan(&id, &logType, &dataJSON, &datePtr, &ledgerName, &idempotencyKey, &idempotencyHash, &sequence)
	if err != nil {
		return nil, err
	}
...
func (c *postgresLogCursor) Next(ctx context.Context) (ledger.Log, error) {
	...
	err := c.rows.Scan(&id, &logType, &dataJSON, &datePtr, &ledgerName, &idempotencyKey, &idempotencyHash, &sequence)
	if err != nil {
...
func (s *PostgresLogStore) GetAllLogs(ctx context.Context, from uint64) (*Cursor[ledger.Log], error) {
	query := `
		SELECT id, type, data, date, ledger, idempotency_key, idempotency_hash, sequence
		FROM logs
	`
...
func (s *PostgresLogStore) GetLastSequenceID(ctx context.Context) (uint64, error) {
