...
func TestProcessProposal_WithIdempotencyKey_NewRequest(t *testing.T) {
	...
	mockStore.EXPECT().PutLedger("test-ledger", gomock.Any(), gomock.Any())
...
func TestProcessCreateLedger(t *testing.T) {
	...
	mockStore.EXPECT().IncrementNextLedgerID().Return(uint32(1))
	mockStore.EXPECT().GetDate().Return(now)
	mockStore.EXPECT().PutLedger("test-ledger", gomock.Any(), gomock.Any()).Do(
		func(name string, info *commonpb.LedgerInfo, boundaries *raftcmdpb.LedgerBoundaries) {
...
func TestProcessCreateLedger_WithMetadata(t *testing.T) {
	...
	mockStore.EXPECT().PutLedgerMetadata(
		data.LedgerMetadataKey{LedgerName: "test-ledger", Key: "env"},
...
func TestProcessAddMetadata_Transaction(t *testing.T) {
	...
	mockStore.EXPECT().GetDate().Return(now)
	mockStore.EXPECT().AddTransactionUpdate(data.TransactionKey{LedgerName: "test-ledger", ID: 5}, gomock.Any()).Do(
...
func TestProcessCreateTransaction(t *testing.T) {
	...
	sourceKey := data.VolumeKey{
		AccountKey: data.AccountKey{LedgerName: "test-ledger", Account: "bank"},
		Asset:      "USD",
	}
...
func TestProcessCreateTransaction_WorldSource(t *testing.T) {
	...
	worldKey := data.VolumeKey{
		AccountKey: data.AccountKey{LedgerName: "test-ledger", Account: "world"},
		Asset:      "USD",
	}
	destKey := data.VolumeKey{
		AccountKey: data.AccountKey{LedgerName: "test-ledger", Account: "users:123"},
...
func TestProcessCreateTransaction_Numscript_SetAccountMeta(t *testing.T) {
	...
	mockStore.EXPECT().PutAccountMetadata(
		data.MetadataKey{
			AccountKey: data.AccountKey{LedgerName: "test-ledger", Account: "users:alice"},
			Key:        "account_type",
		},
