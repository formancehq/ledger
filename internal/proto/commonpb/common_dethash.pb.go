// Code generated by protoc-gen-dethash. DO NOT EDIT.
// source: common.proto

package commonpb

import (
	binary "encoding/binary"
	protohelpers "github.com/planetscale/vtprotobuf/protohelpers"
	maps "maps"
	slices "slices"
)

func (m *Timestamp) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *NullValue) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MetadataValue) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MetadataMap) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *MetadataMap) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.Values) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Values)) {
			v := m.Values[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0xa
		}
	}
	return len(dAtA) - i, nil
}

func (m *ParameterValue) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Uint256) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Posting) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Transaction) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *Transaction) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.RevertedAt != nil {
		size, _ := m.RevertedAt.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x4a
	}
	if m.UpdatedAt != nil {
		size, _ := m.UpdatedAt.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x42
	}
	if m.InsertedAt != nil {
		size, _ := m.InsertedAt.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x3a
	}
	if m.Reverted {
		i--
		if m.Reverted {
			dAtA[i] = 1
		} else {
			dAtA[i] = 0
		}
		i--
		dAtA[i] = 0x30
	}
	if m.Id != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.Id))
		i--
		dAtA[i] = 0x29
	}
	if len(m.Reference) > 0 {
		i -= len(m.Reference)
		copy(dAtA[i:], m.Reference)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Reference)))
		i--
		dAtA[i] = 0x22
	}
	if m.Timestamp != nil {
		size, _ := m.Timestamp.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if len(m.Postings) > 0 {
		for iNdEx := len(m.Postings) - 1; iNdEx >= 0; iNdEx-- {
			size, _ := m.Postings[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0xa
		}
	}
	return len(dAtA) - i, nil
}

func (m *Script) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *Script) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.ContentHash) > 0 {
		i -= len(m.ContentHash)
		copy(dAtA[i:], m.ContentHash)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ContentHash)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Vars) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Vars)) {
			v := m.Vars[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(v)))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if len(m.Plain) > 0 {
		i -= len(m.Plain)
		copy(dAtA[i:], m.Plain)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Plain)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *Volumes) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *VolumesWithBalance) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *VolumesByAssets) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *VolumesByAssets) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.Volumes) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Volumes)) {
			v := m.Volumes[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0xa
		}
	}
	return len(dAtA) - i, nil
}

func (m *PostCommitVolumes) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *PostCommitVolumes) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.VolumesByAccount) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.VolumesByAccount)) {
			v := m.VolumesByAccount[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0xa
		}
	}
	return len(dAtA) - i, nil
}

func (m *Account) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *Account) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.Volumes) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Volumes)) {
			v := m.Volumes[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x32
		}
	}
	if m.UpdatedAt != nil {
		size, _ := m.UpdatedAt.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x2a
	}
	if m.InsertionDate != nil {
		size, _ := m.InsertionDate.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x22
	}
	if m.FirstUsage != nil {
		size, _ := m.FirstUsage.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if len(m.Address) > 0 {
		i -= len(m.Address)
		copy(dAtA[i:], m.Address)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Address)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *TargetAccount) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Target) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MetadataFieldSchema) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MetadataSchema) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *MetadataSchema) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.LedgerFields) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.LedgerFields)) {
			v := m.LedgerFields[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x1a
		}
	}
	if len(m.TransactionFields) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.TransactionFields)) {
			v := m.TransactionFields[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if len(m.AccountFields) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.AccountFields)) {
			v := m.AccountFields[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0xa
		}
	}
	return len(dAtA) - i, nil
}

func (m *SetMetadataFieldTypeCommand) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MetadataIndexID) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *IndexID) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Index) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Idempotency) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *IdempotencyEntry) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Log) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *Log) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.ResponseSignature != nil {
		size, _ := m.ResponseSignature.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x3a
	}
	if len(m.Receipt) > 0 {
		i -= len(m.Receipt)
		copy(dAtA[i:], m.Receipt)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Receipt)))
		i--
		dAtA[i] = 0x32
	}
	if m.Payload != nil {
		size, _ := m.Payload.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if m.Sequence != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.Sequence))
		i--
		dAtA[i] = 0x9
	}
	return len(dAtA) - i, nil
}

func (m *LogPayload) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *LogPayload) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	switch v := m.Type.(type) {
	case *LogPayload_CreateLedger:
		if v.CreateLedger != nil {
			size, _ := v.CreateLedger.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0xa
		}
	case *LogPayload_DeleteLedger:
		if v.DeleteLedger != nil {
			size, _ := v.DeleteLedger.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
		}
	case *LogPayload_Apply:
		if v.Apply != nil {
			size, _ := v.Apply.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1a
		}
	case *LogPayload_RegisterSigningKey:
		if v.RegisterSigningKey != nil {
			size, _ := v.RegisterSigningKey.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x22
		}
	case *LogPayload_RevokeSigningKey:
		if v.RevokeSigningKey != nil {
			size, _ := v.RevokeSigningKey.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x2a
		}
	case *LogPayload_SetSigningConfig:
		if v.SetSigningConfig != nil {
			size, _ := v.SetSigningConfig.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x32
		}
	case *LogPayload_AddedEventsSink:
		if v.AddedEventsSink != nil {
			size, _ := v.AddedEventsSink.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x3a
		}
	case *LogPayload_RemovedEventsSink:
		if v.RemovedEventsSink != nil {
			size, _ := v.RemovedEventsSink.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x42
		}
	case *LogPayload_CloseChapter:
		if v.CloseChapter != nil {
			size, _ := v.CloseChapter.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x4a
		}
	case *LogPayload_SealChapter:
		if v.SealChapter != nil {
			size, _ := v.SealChapter.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x52
		}
	case *LogPayload_ArchiveChapter:
		if v.ArchiveChapter != nil {
			size, _ := v.ArchiveChapter.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x5a
		}
	case *LogPayload_ConfirmArchiveChapter:
		if v.ConfirmArchiveChapter != nil {
			size, _ := v.ConfirmArchiveChapter.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x62
		}
	case *LogPayload_SetMaintenanceMode:
		if v.SetMaintenanceMode != nil {
			size, _ := v.SetMaintenanceMode.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x6a
		}
	case *LogPayload_SetChapterSchedule:
		if v.SetChapterSchedule != nil {
			size, _ := v.SetChapterSchedule.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x72
		}
	case *LogPayload_DeleteChapterSchedule:
		if v.DeleteChapterSchedule != nil {
			size, _ := v.DeleteChapterSchedule.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x7a
		}
	case *LogPayload_PromoteLedger:
		if v.PromoteLedger != nil {
			size, _ := v.PromoteLedger.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0x8a
		}
	case *LogPayload_CreatedPreparedQuery:
		if v.CreatedPreparedQuery != nil {
			size, _ := v.CreatedPreparedQuery.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0x92
		}
	case *LogPayload_UpdatedPreparedQuery:
		if v.UpdatedPreparedQuery != nil {
			size, _ := v.UpdatedPreparedQuery.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0x9a
		}
	case *LogPayload_DeletedPreparedQuery:
		if v.DeletedPreparedQuery != nil {
			size, _ := v.DeletedPreparedQuery.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xa2
		}
	case *LogPayload_SavedNumscript:
		if v.SavedNumscript != nil {
			size, _ := v.SavedNumscript.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xaa
		}
	case *LogPayload_DeletedNumscript:
		if v.DeletedNumscript != nil {
			size, _ := v.DeletedNumscript.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xb2
		}
	case *LogPayload_CreatedQueryCheckpoint:
		if v.CreatedQueryCheckpoint != nil {
			size, _ := v.CreatedQueryCheckpoint.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xba
		}
	case *LogPayload_DeletedQueryCheckpoint:
		if v.DeletedQueryCheckpoint != nil {
			size, _ := v.DeletedQueryCheckpoint.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xc2
		}
	case *LogPayload_SetQueryCheckpointSchedule:
		if v.SetQueryCheckpointSchedule != nil {
			size, _ := v.SetQueryCheckpointSchedule.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xca
		}
	case *LogPayload_DeleteQueryCheckpointSchedule:
		if v.DeleteQueryCheckpointSchedule != nil {
			size, _ := v.DeleteQueryCheckpointSchedule.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xd2
		}
	case *LogPayload_SavedLedgerMetadata:
		if v.SavedLedgerMetadata != nil {
			size, _ := v.SavedLedgerMetadata.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xda
		}
	case *LogPayload_DeletedLedgerMetadata:
		if v.DeletedLedgerMetadata != nil {
			size, _ := v.DeletedLedgerMetadata.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0xe2
		}
	}
	return len(dAtA) - i, nil
}

func (m *PromotedLedgerLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *RegisteredSigningKeyLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *RevokedSigningKeyLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SigningKey) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SetSigningConfigLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AddedEventsSinkLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *RemovedEventsSinkLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SetMaintenanceModeLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *BloomTypeConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ClusterConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *PersistedClusterState) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SetChapterScheduleLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DeletedChapterScheduleLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *CreatedPreparedQueryLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *UpdatedPreparedQueryLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DeletedPreparedQueryLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SavedLedgerMetadataLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *SavedLedgerMetadataLog) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if len(m.Ledger) > 0 {
		i -= len(m.Ledger)
		copy(dAtA[i:], m.Ledger)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Ledger)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *DeletedLedgerMetadataLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *NumscriptInfo) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SavedNumscriptLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DeletedNumscriptLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SetQueryCheckpointScheduleLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DeletedQueryCheckpointScheduleLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *CreatedQueryCheckpointLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DeletedQueryCheckpointLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SinkConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SinkStatus) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SinkError) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *NatsSinkConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ClickHouseSinkConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *KafkaSinkConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *HttpSinkConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DatabricksSinkConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DatabricksOAuthM2M) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *CreatedLedgerLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *CreatedLedgerLog) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.Id != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Id))
		i--
		dAtA[i] = 0x40
	}
	if m.DefaultEnforcementMode != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.DefaultEnforcementMode))
		i--
		dAtA[i] = 0x38
	}
	if len(m.AccountTypes) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.AccountTypes)) {
			v := m.AccountTypes[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x32
		}
	}
	if m.MirrorSource != nil {
		size, _ := m.MirrorSource.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x2a
	}
	if m.Mode != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Mode))
		i--
		dAtA[i] = 0x20
	}
	if m.MetadataSchema != nil {
		size, _ := m.MetadataSchema.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if m.CreatedAt != nil {
		size, _ := m.CreatedAt.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Name) > 0 {
		i -= len(m.Name)
		copy(dAtA[i:], m.Name)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Name)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *DeletedLedgerLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ApplyLedgerLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *ApplyLedgerLog) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.Log != nil {
		size, _ := m.Log.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if len(m.LedgerName) > 0 {
		i -= len(m.LedgerName)
		copy(dAtA[i:], m.LedgerName)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.LedgerName)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *LedgerLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *LedgerLog) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.PurgedVolumes) > 0 {
		for iNdEx := len(m.PurgedVolumes) - 1; iNdEx >= 0; iNdEx-- {
			size, _ := m.PurgedVolumes[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x22
		}
	}
	if m.Id != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.Id))
		i--
		dAtA[i] = 0x19
	}
	if m.Date != nil {
		size, _ := m.Date.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if m.Data != nil {
		size, _ := m.Data.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *TouchedVolume) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *LedgerLogPayload) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *LedgerLogPayload) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	switch v := m.Payload.(type) {
	case *LedgerLogPayload_CreatedTransaction:
		if v.CreatedTransaction != nil {
			size, _ := v.CreatedTransaction.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0xa
		}
	case *LedgerLogPayload_RevertedTransaction:
		if v.RevertedTransaction != nil {
			size, _ := v.RevertedTransaction.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
		}
	case *LedgerLogPayload_SavedMetadata:
		if v.SavedMetadata != nil {
			size, _ := v.SavedMetadata.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x1a
		}
	case *LedgerLogPayload_DeletedMetadata:
		if v.DeletedMetadata != nil {
			size, _ := v.DeletedMetadata.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x22
		}
	case *LedgerLogPayload_SetMetadataFieldType:
		if v.SetMetadataFieldType != nil {
			size, _ := v.SetMetadataFieldType.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x2a
		}
	case *LedgerLogPayload_RemovedMetadataFieldType:
		if v.RemovedMetadataFieldType != nil {
			size, _ := v.RemovedMetadataFieldType.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x32
		}
	case *LedgerLogPayload_FillGap:
		if v.FillGap != nil {
			size, _ := v.FillGap.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x4a
		}
	case *LedgerLogPayload_CreateIndex:
		if v.CreateIndex != nil {
			size, _ := v.CreateIndex.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x52
		}
	case *LedgerLogPayload_DropIndex:
		if v.DropIndex != nil {
			size, _ := v.DropIndex.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x5a
		}
	case *LedgerLogPayload_AddedAccountType:
		if v.AddedAccountType != nil {
			size, _ := v.AddedAccountType.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x6a
		}
	case *LedgerLogPayload_RemovedAccountType:
		if v.RemovedAccountType != nil {
			size, _ := v.RemovedAccountType.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x72
		}
	case *LedgerLogPayload_UpdatedDefaultEnforcementMode:
		if v.UpdatedDefaultEnforcementMode != nil {
			size, _ := v.UpdatedDefaultEnforcementMode.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x7a
		}
	}
	return len(dAtA) - i, nil
}

func (m *CreatedIndexLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *DroppedIndexLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *FilledGapLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *CreatedTransaction) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *CreatedTransaction) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.PostCommitVolumes != nil {
		size, _ := m.PostCommitVolumes.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x22
	}
	if m.ChapterId != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.ChapterId))
		i--
		dAtA[i] = 0x19
	}
	if len(m.AccountMetadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.AccountMetadata)) {
			v := m.AccountMetadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if m.Transaction != nil {
		size, _ := m.Transaction.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *RevertedTransaction) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *RevertedTransaction) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.PostCommitVolumes != nil {
		size, _ := m.PostCommitVolumes.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if m.RevertTransaction != nil {
		size, _ := m.RevertTransaction.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if m.RevertedTransactionId != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.RevertedTransactionId))
		i--
		dAtA[i] = 0x9
	}
	return len(dAtA) - i, nil
}

func (m *SavedMetadata) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *SavedMetadata) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if m.Target != nil {
		size, _ := m.Target.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *DeletedMetadata) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SetMetadataFieldTypeLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *RemovedMetadataFieldTypeLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Chapter) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ClosedChapterLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SealedChapterLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ArchivedChapterLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ConfirmedArchiveChapterLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MirrorSourceConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *HttpMirrorSourceConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *OAuth2ClientCredentials) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *PostgresMirrorSourceConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MirrorSyncError) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *MirrorSyncProgress) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *LedgerInfo) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *LedgerInfo) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.Id != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Id))
		i--
		dAtA[i] = 0x68
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x62
		}
	}
	if m.DefaultEnforcementMode != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.DefaultEnforcementMode))
		i--
		dAtA[i] = 0x58
	}
	if len(m.AccountTypes) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.AccountTypes)) {
			v := m.AccountTypes[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x52
		}
	}
	if m.MirrorSyncProgress != nil {
		size, _ := m.MirrorSyncProgress.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x3a
	}
	if m.MirrorSource != nil {
		size, _ := m.MirrorSource.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x32
	}
	if m.Mode != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Mode))
		i--
		dAtA[i] = 0x28
	}
	if m.MetadataSchema != nil {
		size, _ := m.MetadataSchema.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x22
	}
	if m.DeletedAt != nil {
		size, _ := m.DeletedAt.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if m.CreatedAt != nil {
		size, _ := m.CreatedAt.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Name) > 0 {
		i -= len(m.Name)
		copy(dAtA[i:], m.Name)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Name)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *SaveMetadataCommand) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *SaveMetadataCommand) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
	if m.Target != nil {
		size, _ := m.Target.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *DeleteMetadataCommand) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *TransactionState) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *TransactionState) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.Timestamp != nil {
		size, _ := m.Timestamp.MarshalToSizedBufferVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x22
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x1a
		}
	}
	if m.RevertedByTransaction != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.RevertedByTransaction))
		i--
		dAtA[i] = 0x11
	}
	if m.CreatedByLog != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.CreatedByLog))
		i--
		dAtA[i] = 0x9
	}
	return len(dAtA) - i, nil
}

func (m *IdempotencyKeyValue) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *IdempotencyKeyValue) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.LogCount != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.LogCount))
		i--
		dAtA[i] = 0x30
	}
	if m.Failure != nil {
		size, _ := m.Failure.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x2a
	}
	if m.CreatedAt != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.CreatedAt))
		i--
		dAtA[i] = 0x21
	}
	if m.HashVersion != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.HashVersion))
		i--
		dAtA[i] = 0x18
	}
	if len(m.Hash) > 0 {
		i -= len(m.Hash)
		copy(dAtA[i:], m.Hash)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Hash)))
		i--
		dAtA[i] = 0x12
	}
	if m.FirstLogSequence != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(m.FirstLogSequence))
		i--
		dAtA[i] = 0x9
	}
	return len(dAtA) - i, nil
}

func (m *IdempotencyFailure) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *IdempotencyFailure) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.Metadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.Metadata)) {
			v := m.Metadata[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(v)))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x1a
		}
	}
	if len(m.Message) > 0 {
		i -= len(m.Message)
		copy(dAtA[i:], m.Message)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Message)))
		i--
		dAtA[i] = 0x12
	}
	if m.Reason != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Reason))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
}

func (m *TransactionReferenceValue) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AccountState) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *NumscriptVersionValue) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *SegmentType) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *UUIDConstraint) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *Uint64Constraint) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *BytesConstraint) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AccountType) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *AccountType) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.DefaultMetadata) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.DefaultMetadata)) {
			v := m.DefaultMetadata[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x2a
		}
	}
	if len(m.SegmentTypes) > 0 {
		for _, k := range slices.Sorted(maps.Keys(m.SegmentTypes)) {
			v := m.SegmentTypes[k]
			baseI := i
			size, _ := v.MarshalToSizedBufferVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i -= len(k)
			copy(dAtA[i:], k)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = protohelpers.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x22
		}
	}
	if m.Persistence != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Persistence))
		i--
		dAtA[i] = 0x18
	}
	if len(m.Pattern) > 0 {
		i -= len(m.Pattern)
		copy(dAtA[i:], m.Pattern)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Pattern)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Name) > 0 {
		i -= len(m.Name)
		copy(dAtA[i:], m.Name)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Name)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *AddedAccountTypeLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *AddedAccountTypeLog) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if m.AccountType != nil {
		size, _ := m.AccountType.MarshalToSizedBufferDeterministicVT(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *RemovedAccountTypeLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *UpdatedDefaultEnforcementModeLog) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *QueryFilter) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ReferenceCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *LedgerCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *LogIdCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *BuiltinUintCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *LogBuiltinUintCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AccountHasAssetCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AndFilter) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *OrFilter) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *NotFilter) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *FieldRef) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *FieldCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *StringCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *IntCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *UintCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *BoolCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ExistsCondition) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AddressMatch) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *PreparedQuery) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AggregatedVolume) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AggregateResult) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *GroupedAggregateResult) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *PreparedQueryCursor) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	sz := m.SizeVT()
	buf := make([]byte, sz)
	n, _ := m.MarshalToSizedBufferDeterministicVT(buf)
	return append(dAtA, buf[sz-n:]...)
}

func (m *PreparedQueryCursor) MarshalToSizedBufferDeterministicVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	if m.unknownFields != nil {
		i -= len(m.unknownFields)
		copy(dAtA[i:], m.unknownFields)
	}
	if len(m.TransactionData) > 0 {
		for iNdEx := len(m.TransactionData) - 1; iNdEx >= 0; iNdEx-- {
			size, _ := m.TransactionData[iNdEx].MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x32
		}
	}
	if len(m.AccountData) > 0 {
		for iNdEx := len(m.AccountData) - 1; iNdEx >= 0; iNdEx-- {
			size, _ := m.AccountData[iNdEx].MarshalToSizedBufferDeterministicVT(dAtA[:i])
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x2a
		}
	}
	if len(m.Next) > 0 {
		i -= len(m.Next)
		copy(dAtA[i:], m.Next)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Next)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.Previous) > 0 {
		i -= len(m.Previous)
		copy(dAtA[i:], m.Previous)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Previous)))
		i--
		dAtA[i] = 0x1a
	}
	if m.HasMore {
		i--
		if m.HasMore {
			dAtA[i] = 1
		} else {
			dAtA[i] = 0
		}
		i--
		dAtA[i] = 0x10
	}
	if m.PageSize != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.PageSize))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
}

func (m *LedgerStats) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *PersistedConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *CallerIdentity) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *CallerSnapshot) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *S3StorageConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *AzureStorageConfig) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *BackupStorage) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ReadOptions) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}

func (m *ListOptions) MarshalDeterministicVT(dAtA []byte) []byte {
	if m == nil {
		return dAtA
	}
	b, err := m.MarshalVT()
	if err != nil {
		panic("MarshalDeterministicVT: " + err.Error())
	}
	return append(dAtA, b...)
}
