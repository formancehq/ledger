...
const (
	...
	// (every non-system account ever referenced as source/destination). A marker
	// with no matching referenced account — or a referenced account with no
	// marker once the ledger is READY — is tampering. The account_defaults_status
	// SEEDING→READY flip rides a non-audited TechnicalUpdate, so the verifier
	// covers marker presence, not the status field.
