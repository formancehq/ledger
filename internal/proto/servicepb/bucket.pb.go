...
const (
	...
	// Emitted when the SubAttrAccount existence-marker projection diverges from
	// the account set the checker re-derived by replaying transaction postings
	// (every non-system account ever referenced as source/destination). Both
	// directions are tampering: a marker with no matching referenced account, and
	// an account the replay recorded as touched on a defaults-bearing ledger that
	// has no live marker (a missing marker re-applies default metadata on the next
	// touch). The reverse direction needs no activation status — recordTouched
	// only records touches while the ledger declares default-bearing account
	// types, so the replay-touched set is exactly the set that must carry markers.
	CheckStoreErrorType_CHECK_STORE_ERROR_TYPE_ACCOUNT_MISMATCH CheckStoreErrorType = 11
)
...
var (
	CheckStoreErrorType_name = map[int32]string{
		...
		11: "CHECK_STORE_ERROR_TYPE_ACCOUNT_MISMATCH",
	}
	CheckStoreErrorType_value = map[string]int32{
		...
		"CHECK_STORE_ERROR_TYPE_ACCOUNT_MISMATCH":            11,
	}
)
...
const file_bucket_proto_rawDesc = "" +
	...
	"\fcommit_index\x18\x01 \x01(\x06R\vcommitIndex*\xba\x04\n" +
	...
	"\x12+\n" +
	"'CHECK_STORE_ERROR_TYPE_ACCOUNT_MISMATCH\x10\v*W\n" +
