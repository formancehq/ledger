...
// SpoolInterceptor wraps a Spool and allows intercepting method calls.
// All interceptors are optional - if not set, the call passes through to the underlying Spool.
type SpoolInterceptor struct {
	delegate Spool
	mu       sync.RWMutex
...
// NewSpoolInterceptor creates a new SpoolInterceptor wrapping the given Spool.
func NewSpoolInterceptor(delegate Spool) *SpoolInterceptor {
	return &SpoolInterceptor{
		delegate: delegate,
	}
}

// Delegate returns the underlying Spool.
func (s *SpoolInterceptor) Delegate() Spool {
	return s.delegate
}

// AppendCommittedEntries implements Spool.
func (s *SpoolInterceptor) AppendCommittedEntries(ctx context.Context, entries ...raftpb.Entry) error {
	s.mu.RLock()
...
func (s *SpoolInterceptor) End() (*Position, error) {
	s.mu.RLock()
	interceptor := s.OnEnd
	s.mu.RUnlock()
...
func (s *SpoolInterceptor) ReplayUntil(
	ctx context.Context,
	end Position,
	lastApplied uint64,
	applyFn func(raftpb.Entry) error,
...
func (s *SpoolInterceptor) Prune(lastApplied uint64) error {
	s.mu.RLock()
...
func (s *SpoolInterceptor) Close() error {
	s.mu.RLock()
...
func (s *SpoolInterceptor) SetAppendCommittedEntriesInterceptor(
...
func (s *SpoolInterceptor) SetEndInterceptor(fn func(delegate Spool) (*Position, error)) {
	s.mu.Lock()
	s.OnEnd = fn
	s.mu.Unlock()
}
...
func (s *SpoolInterceptor) SetReplayUntilInterceptor(
...
func (s *SpoolInterceptor) SetPruneInterceptor(fn func(delegate Spool, lastApplied uint64) error) {
	s.mu.Lock()
	s.OnPrune = fn
	s.mu.Unlock()
}
...
func (s *SpoolInterceptor) SetCloseInterceptor(fn func(delegate Spool) error) {
	s.mu.Lock()
	s.OnClose = fn
	s.mu.Unlock()
}
...
func (s *SpoolInterceptor) ClearInterceptors() {
	s.mu.Lock()
...
// Ensure SpoolInterceptor implements Spool.
var _ Spool = (*SpoolInterceptor)(nil)
