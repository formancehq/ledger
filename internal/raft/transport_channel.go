...
func NewChannelTransport(nodeID uint64, config ChannelTransportConfig) *ChannelTransport {
	return &ChannelTransport{
		nodeID:               nodeID,
		highPriorityRecvCh:   make(chan []raftpb.Message, config.RecvBufferSize[0]),
		mediumPriorityRecvCh: make(chan []raftpb.Message, config.RecvBufferSize[1]),
		lowPriorityRecvCh:    make(chan []raftpb.Message, config.RecvBufferSize[2]),
...
func (t *ChannelTransport) sendBatch(peerID uint64, priority int, msgs []raftpb.Message) {
	...
	switch priority {
	case 0:
		ch = peer.highPriorityRecvCh
	case 1:
		ch = peer.mediumPriorityRecvCh
	default:
		ch = peer.lowPriorityRecvCh
	}
