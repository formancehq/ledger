...
// TestProcessTickRebuildsOnRestoreGenerationChange is the regression for the
// catch-up race the cursor>head signal cannot catch: the primary store is
// restored beneath the cursor, then the audit head re-grows PAST the old cursor
// before the tick re-samples — so cursorAheadOfHead is false and the gap is
// under the (default-disabled) threshold. The restore-generation change is the
// only remaining rollback signal, and processTick must rebuild on it.
func TestProcessTickRebuildsOnRestoreGenerationChange(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	idx, mainStore, rs := newIndexerForTest(t)

	// Seed and index entries 1..5, leaving cursor at 5.
	for s := uint64(1); s <= 5; s++ {
		writeAuditEntry(t, mainStore, &auditpb.AuditEntry{
			Sequence: s, ProposalId: s, Timestamp: &commonpb.Timestamp{Data: s * 1_000_000},
			Outcome: &auditpb.AuditEntry_Success{Success: &auditpb.AuditSuccess{}},
	...
	// boot seeds the restore generation; ProcessOnce indexes 1..5.
	require.NoError(t, idx.boot(ctx))
	cursor, err := rs.ReadAuditProgress()
	require.NoError(t, err)
	require.Equal(t, uint64(5), cursor)

	// Simulate the rollback+catch-up: a real RestoreCheckpoint bumps the store
	// generation (the runtime follower-sync signature), and the post-restore
	// head re-grows to 10 — strictly ABOVE the cursor (5), so the position
	// signal cannot fire.
	checkpointID, err := mainStore.CreateSnapshot()
	require.NoError(t, err)
	require.NoError(t, mainStore.RestoreCheckpoint(checkpointID))
	...
	require.False(t, cursorAheadOfHead(5, last), "head re-grew past cursor: position signal is inert")
	require.False(t, idx.shouldRebuildOnBoot(5, last), "gap under default-disabled threshold: no position signal")

	// processTick must still rebuild — driven purely by the generation change —
	// and re-index the full surviving chain from 0.
	require.NoError(t, idx.processTick(ctx))

	cursor, err = rs.ReadAuditProgress()
	require.NoError(t, err)
	require.Equal(t, uint64(10), cursor, "rebuild re-indexes the full surviving chain")

	// A second tick with no further restore must NOT rebuild (generation resync).
	require.NoError(t, idx.processTick(ctx))
	cursor, err = rs.ReadAuditProgress()
	require.NoError(t, err)
	require.Equal(t, uint64(10), cursor, "no rebuild without a new restore")
}
