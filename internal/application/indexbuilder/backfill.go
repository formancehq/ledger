...
func (b *Builder) addSchemaRewriteTask(cfg *ledgerIndexConfig, ledgerName string, smft *commonpb.SetMetadataFieldTypeLog) error {
	...
	for _, bt := range b.backfillTasks {
		...
		if err := b.bumpPendingVersion(ledgerName, indexID, smft.GetType(), smft.GetRevision(), smft.GetIncarnation()); err != nil {
...
func (b *Builder) bumpPendingVersion(ledgerName string, indexID *commonpb.IndexID, toType commonpb.MetadataType, toRevision uint32, toIncarnation uint64) error {
	...
	newState := readstore.IndexVersionState{
		...
		CurrentIncarnation:  prior.CurrentIncarnation,
		PendingType:         toType,
		PendingTypeDeclared: true,
		PendingRevision:     toRevision,
		PendingIncarnation:  toIncarnation,
	}
