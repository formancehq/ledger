...
func TestBindingRevisionWalk(t *testing.T) {
	...
	require.Equal(t, uint64(7), st.CurrentIncarnation, "and the stamped declaration lineage")

	// Retype to revision 2: the pending slot carries the SMFT's stamp.
	batch = b.readStore.NewBatch()
	b.initBatch(batch)
	require.NoError(t, b.bumpPendingVersion(ledger, id, commonpb.MetadataType_METADATA_TYPE_STRING, 2, 7))
	require.NoError(t, b.wb.Flush())
	...
	require.Equal(t, uint64(7), st.CurrentIncarnation, "a retype stays in the same lineage")
	require.Equal(t, uint64(7), st.PendingIncarnation)
	...
	require.Equal(t, uint64(7), st.CurrentIncarnation, "and the pending lineage with it")
	require.Zero(t, st.PendingIncarnation)
}
