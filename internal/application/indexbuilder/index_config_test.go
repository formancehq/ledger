...
func TestHandleIndexReadyLog_TxBuiltin(t *testing.T) {
	t.Parallel()

	b := &Builder{indexConfig: make(map[string]*ledgerIndexConfig)}

	b.handleIndexReadyLog("ledger1", &commonpb.IndexReadyLog{
...
func TestHandleIndexReadyLog_TxMetadata(t *testing.T) {
	t.Parallel()
	...
	b.handleIndexReadyLog("ledger1", &commonpb.IndexReadyLog{
		Index: &commonpb.IndexReadyLog_Transaction{
			Transaction: &commonpb.TransactionIndex{
				Kind: &commonpb.TransactionIndex_MetadataKey{MetadataKey: "tag"},
			},
		},
	})
	...
	cfg := b.indexConfig["ledger1"]
	require.NotNil(t, cfg)
	assert.True(t, cfg.txMetadataIndexed["tag"])
}

func TestHandleIndexReadyLog_AcctBuiltin(t *testing.T) {
	t.Parallel()

	b := &Builder{indexConfig: make(map[string]*ledgerIndexConfig)}

	b.handleIndexReadyLog("ledger1", &commonpb.IndexReadyLog{
		Index: &commonpb.IndexReadyLog_Account{
			Account: &commonpb.AccountIndex{
				Kind: &commonpb.AccountIndex_Builtin{
					Builtin: commonpb.AccountBuiltinIndex_ACCT_BUILTIN_INDEX_UNSPECIFIED,
				},
...
func TestHandleIndexReadyLog_AcctMetadata(t *testing.T) {
	t.Parallel()
	...
	b.handleIndexReadyLog("ledger1", &commonpb.IndexReadyLog{
		Index: &commonpb.IndexReadyLog_Account{
			Account: &commonpb.AccountIndex{
				Kind: &commonpb.AccountIndex_MetadataKey{MetadataKey: "type"},
			},
		},
	})
	...
	cfg := b.indexConfig["ledger1"]
	require.NotNil(t, cfg)
	assert.True(t, cfg.acctMetadataIndexed["type"])
}

func TestHandleIndexReadyLog_LogBuiltin(t *testing.T) {
	t.Parallel()

	b := &Builder{indexConfig: make(map[string]*ledgerIndexConfig)}

	b.handleIndexReadyLog("ledger1", &commonpb.IndexReadyLog{
		Index: &commonpb.IndexReadyLog_LogBuiltin{
			LogBuiltin: commonpb.LogBuiltinIndex_LOG_BUILTIN_INDEX_DATE,
		},
	})
...
func TestHandleIndexReadyLog_RemovesBackfillTask(t *testing.T) {
	t.Parallel()
	...
	// Mark as ready should remove the backfill task.
	b.handleIndexReadyLog("ledger1", &commonpb.IndexReadyLog{
		Index: &commonpb.IndexReadyLog_Transaction{
			Transaction: &commonpb.TransactionIndex{
				Kind: &commonpb.TransactionIndex_Builtin{
