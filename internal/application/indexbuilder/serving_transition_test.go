...
func newKillableTestBuilder(t *testing.T) *Builder {
	t.Helper()
...
// disabled, carries no memtable — what a hard kill keeps.
...
func TestCompleteBackfill_PromotionSurvivesAKill(t *testing.T) {
	...
	assert.False(t, b.readStore.ServingTransitionInFlight(servingTestLedger, canonical), "the flush completed, so nothing is left in flight")
...
func TestScanCompleteSwitch_PromotionSurvivesAKill(t *testing.T) {
	...
	assert.False(t, b.readStore.ServingTransitionInFlight(servingTestLedger, canonical))
...
func requirePromotionHeldUntilRetry(t *testing.T, b *Builder, canonical string) {
	t.Helper()

	assert.True(t, b.readStore.ServingTransitionInFlight(servingTestLedger, canonical), "a failed flush must keep the mark")
	...
	b.readStore.OverrideServingFlushForTest(nil)
	b.retryServingFlush()
	assert.False(t, b.readStore.ServingTransitionInFlight(servingTestLedger, canonical), "the retry lifts the mark")
...
func failNextFlush(b *Builder) {
	b.readStore.OverrideServingFlushForTest(func() error { return errors.New("disk on fire") })
}
...
func TestScanCompleteSwitch_FlushFailureKeepsThePromotionRefused(t *testing.T) {
	...
	requirePromotionHeldUntilRetry(t, b, canonical)
}
