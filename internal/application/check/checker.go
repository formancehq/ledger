					...
					if payload.Apply.GetLog() != nil && payload.Apply.GetLog().GetData() != nil {
						...
						case *commonpb.LedgerLogPayload_SetMetadataFieldType:
							if l := d.SetMetadataFieldType; l != nil {
								setExpectedSchemaField(expectedSchemas, ledgerName, l.GetTargetType(), l.GetKey(), &commonpb.MetadataFieldSchema{
									Type:        l.GetType(),
									Revision:    l.GetRevision(),
									Incarnation: l.GetIncarnation(),
								})
							}
...
// compareIndexesScope carries the stored projection under judgement plus the
...
func setExpectedSchemaField(schemas map[string]*commonpb.MetadataSchema, ledger string, target commonpb.TargetType, key string, declared *commonpb.MetadataFieldSchema) {
...
func fieldTypesEqual(a, b map[string]*commonpb.MetadataFieldSchema) bool {
	...
	for k, av := range a {
		bv, ok := b[k]
		if !ok || av.GetType() != bv.GetType() || av.GetRevision() != bv.GetRevision() || av.GetIncarnation() != bv.GetIncarnation() {
			return false
		}
	}
