...
// TestCompareSchema_IncarnationDiverges: type and revision agree, but the
// stored declaration belongs to a different lineage than the audit derived —
// a restore or rebuild that re-minted the incarnation instead of preserving
// the log's stamp. Left undetected it disarms the cross-incarnation half of
// the serving-window gate.
func TestCompareSchema_IncarnationDiverges(t *testing.T) {
	t.Parallel()

	stored := accountFieldSchema("tier", commonpb.MetadataType_METADATA_TYPE_STRING)
	stored.AccountFields["tier"].Revision = 2
	stored.AccountFields["tier"].Incarnation = 41
	...
	expected := accountFieldSchema("tier", commonpb.MetadataType_METADATA_TYPE_STRING)
	expected.AccountFields["tier"].Revision = 2
	expected.AccountFields["tier"].Incarnation = 7
