...
func TestVerifyAuditHashChain_SigningFoldIgnoresLegacyReplayReferences(t *testing.T) {
	...
	_, err = checker.verifyAuditHashChain(context.Background(), handle, newChainBoundState(), verifier, newClusterPolicyVerifier(), newLogBoundsVerifier(), func(*servicepb.CheckStoreEvent) {})
	require.NoError(t, err)
