...
func TestDefaultController_InspectIndex_ServingWindowGate(t *testing.T) {
	...
	newInspectControllerIn := func(t *testing.T, bindingRevision uint32, bindingIncarnation uint64) *DefaultController {
		t.Helper()
		...
		require.NoError(t, state.SaveLedger(batch, ledger, &commonpb.LedgerInfo{
			...
			MetadataSchema: &commonpb.MetadataSchema{
				AccountFields: map[string]*commonpb.MetadataFieldSchema{
					key: {Type: commonpb.MetadataType_METADATA_TYPE_INT8, Revision: 3, Incarnation: 7},
				},
			},
		}))
		...
		require.NoError(t, rs.WriteIndexVersionState(rsBatch, ledger, indexes.Canonical(indexID), readstore.IndexVersionState{
			...
			CurrentIncarnation:  bindingIncarnation,
		}))
		require.NoError(t, rsBatch.Commit())

		return NewDefaultController(nil, store, logger, attrs, rs, nil, meter)
	}

	// The schema's lineage is 7; a binding in it is judged by distance.
	newInspectController := func(t *testing.T, bindingRevision uint32) *DefaultController {
		return newInspectControllerIn(t, bindingRevision, 7)
	}
	...
	t.Run("a dropped incarnation's binding refuses even at a serveable distance", func(t *testing.T) {
		t.Parallel()

		// Revision 3 against a schema at revision 3 is "converged" by
		// distance; a different lineage means the binding was built for a
		// declaration the schema has since replaced, so the keyspace it names
		// no longer exists.
		c := newInspectControllerIn(t, 3, 41)
