...
func RestoreModule() fx.Option {
	return fx.Options(
		...
		fx.Invoke(
			...
			func(serviceServer *ServiceServer) {
				hs := health.NewServer()
				healthpb.RegisterHealthServer(serviceServer.GetServer(), hs)
				hs.SetServingStatus("", healthpb.HealthCheckResponse_SERVING)
			},
			// Register RestoreService on ServiceServer
			func(serviceServer *ServiceServer, restoreServer *RestoreServiceServerImpl) {
				RegisterRestoreService(serviceServer.GetServer(), restoreServer)
			},
			// Start ServiceServer
			func(
				lc fx.Lifecycle,
