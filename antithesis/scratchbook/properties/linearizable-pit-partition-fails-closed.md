...
| **Antithesis Angle** | Keep workload-to-node gRPC and the history object store reachable while partitioning Raft links: isolate the current leader from both followers, isolate one follower as a minority, and separately leave a two-node majority connected. Start each probe only after the intended cut is active, use a short deadline with configured retries disabled plus an authenticated server acknowledgement, then heal and require exact success from every voter bracketed by stable leader-reported membership snapshots. |
...
- `tests/antithesis/workload/internal/pernode.go` preserves its historical retry mode, while the property uses `grpc.WithDisableRetry` plus no service retry policy or workload retry interceptor. grpc-go may still transparently retry before server processing, so only an exact SUT acknowledgement under stable pre/post membership can satisfy coverage.
...
- `antithesis/scratchbook/existing-assertions.md` records the tagged route signal that distinguishes a default linearizable barrier stop from a packet-level failure.
...
- **Implemented — exact-node access:** `DialPerNode(ctx, retryEnabled)` preserves the existing retrying callers and supplies a direct mode that sets `grpc.WithDisableRetry`, omits the service retry policy and omits both workload retry interceptors. Transparent pre-processing retries remain a grpc-go behavior, so the property refuses to count an attempt without the exact server acknowledgement.
- **Implemented — repository-controlled no-quorum execution:** `singleton_driver_quorum_recovery` creates an isolated one-posting PIT ledger, waits for every voter in a stable snapshot to serve it exactly, deletes both non-leader pods, scales to one, and probes only while Kubernetes exposes the original leader as the sole Ledger pod and both the immediately-before and immediately-after local membership samples still report three voters.
- **Implemented — PIT trailer/oracle helper:** `CheckLinearizablePIT` removes `x-consistency`, requires the exact immutable-view trailer, authenticates ledger incarnation/selector/token, enforces the acknowledged log floor, rejects duplicate buckets, and compares the full unfiltered aggregate to the independent `2^64 + 1` monetary oracle.
- **Implemented — strict outcome classifier:** `IsLinearizablePITPartitionTransient` admits only reasonless `Unavailable`/`DeadlineExceeded` and `HISTORY_BUILDING`/`HISTORY_BEHIND`; permanent PIT, external-service, filtered read-index and unrelated codes fail the safety assertion.
- **Implemented — SUT reachability:** the Antithesis-tagged route gives tagged probes a one-second internal barrier deadline, then emits an authenticated response header and `pit: aggregate stopped at default linearizable read barrier` only after the barrier failed while the local node still leads three voters. The client deadline remains longer, so the header is observable before response completion.
- **Implemented — post-repair liveness:** after force-removal and scale-up, two consecutive leader snapshots must have the same leader and voter IDs; fresh direct connections then serve one exact default-consistency PIT result for every voter ID, and a second stable snapshot must still match after the sequential probes. A changed set restarts the bounded recovery loop.
- **Missing — selective-link matrix:** stopped followers cover the live-node/no-quorum specialization only. Named Raft-only leader/minority/majority cuts still require tenant fault-controller support that preserves workload gRPC and MinIO.
