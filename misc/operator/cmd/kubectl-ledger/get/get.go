...
func runGet(cmd *cobra.Command, opts *cmdutil.Options, args []string) error {
	...
	cmdutil.RenderTable(
		...
		[][]string{
			...
			{"Cluster ID", ledger.Spec.ClusterID},
	...
	// Networking section
	pterm.DefaultSection.Println("Networking")

	netRows := [][]string{
		{"HTTP Port", strconv.Itoa(int(ledger.Spec.HttpPort))},
		{"gRPC Port", strconv.Itoa(int(ledger.Spec.GrpcPort))},
	}

	if ledger.Status.Endpoints != nil {
		ep := ledger.Status.Endpoints
		netRows = append(netRows, []string{"HTTP Endpoint", ep.HTTP})
		netRows = append(netRows, []string{"gRPC Endpoint", ep.GRPC})
		if ep.External {
			netRows = append(netRows, []string{"External", pterm.Green("yes")})
		} else {
			netRows = append(netRows, []string{"External", "no (cluster-internal)"})
		}
	}

	if ledger.Spec.Ingress != nil && ledger.Spec.Ingress.Enabled {
		for _, h := range ledger.Spec.Ingress.Hosts {
			netRows = append(netRows, []string{"Ingress HTTP", h.Host})
		}
	}
	if ledger.Spec.IngressGrpc != nil && ledger.Spec.IngressGrpc.Enabled {
		for _, h := range ledger.Spec.IngressGrpc.Hosts {
			netRows = append(netRows, []string{"Ingress gRPC", h.Host})
		}
	}
	cmdutil.RenderTable([]string{"KEY", "VALUE"}, netRows)
