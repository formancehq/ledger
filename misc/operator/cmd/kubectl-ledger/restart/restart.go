...
func NewCommand(opts *cmdutil.Options) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:   "Rolling restart of a Cluster deployment",
		Long:    "Triggers a rolling restart by patching podAnnotations on the Cluster CR. The operator detects the spec hash change and performs a StatefulSet rolling update.",
...
func runRestart(cmd *cobra.Command, opts *cmdutil.Options, f *restartFlags, args []string) error {
	...
	if !f.yes {
		confirm, err := cmdutil.PromptConfirm(
			fmt.Sprintf("Rolling restart Cluster %s?", pterm.Cyan(name)),
			true,
		)
		if err != nil {
			return err
	...
	spinner.Success("Rolling restart triggered for Cluster " + pterm.Cyan(name))
