...
func TestReconcile_NetworkPolicyViaDefaults(t *testing.T) {
	ns := createTestNamespace(t)

	// Create cluster-scoped LedgerDefaults with NetworkPolicy enabled.
	defaults := newLedgerDefaults("defaults-np")
	defaults.Spec.NetworkPolicy = &ledgerv1alpha1.NetworkPolicySpec{
		Enabled: true,
	}
	...
	// Create LedgerService referencing defaults (no local NetworkPolicy).
	ls := newLedgerService("np-defaults", ns)
	ls.Spec.DefaultsRef = "defaults-np"
	require.NoError(t, k8sClient.Create(ctx, ls))

	// NetworkPolicy should be created from defaults.
	np := &networkingv1.NetworkPolicy{}
	requireEventually(t, func() bool {
		return k8sClient.Get(ctx, types.NamespacedName{Name: "np-defaults", Namespace: ns}, np) == nil
	}, "NetworkPolicy should be created from LedgerDefaults")

	require.Len(t, np.Spec.Egress, 3)
	assert.Equal(t, defaultExternalCIDRExcept, np.Spec.Egress[2].To[0].IPBlock.Except)
	requireOwnerRef(t, np.OwnerReferences, "np-defaults")
}
